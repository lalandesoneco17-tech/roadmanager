...
const loadData=async()=>{const _localTs=localTs();if(sb){try{const{data:row,error}=await sb.from('app_data').select('data,updated_at').eq('id','main').single();if(!error&&row&&row.data&&Object.keys(row.data).length>0){const serverTs=row.updated_at?new Date(row.updated_at).getTime():0;if(_localTs&&_localTs>serverTs+3000){const local=localLoad();if(local){console.warn('Local plus recent que Supabase ('+(Math.round((_localTs-serverTs)/1000))+'s) → push local et utilise local');try{await sb.from('app_data').upsert({id:'main',data:local,updated_at:new Date(_localTs).toISOString()});console.log('Push local OK')}catch(e){console.warn('Push local failed:',e)}return local}}const merged={...defaultData(),...row.data};localSave(merged);console.log('Loaded from Supabase');return merged}}catch(e){console.warn('Supabase load failed, fallback localStorage',e)}}const local=localLoad();if(local){if(sb){try{await sb.from('app_data').upsert({id:'main',data:local,updated_at:new Date().toISOString()});console.log('Migrated localStorage to Supabase')}catch(e){console.warn('Migration to Supabase failed',e)}}return local}return defaultData()};
...
if(typeof window!=='undefined'&&!window.__teOnlineBound){window.__teOnlineBound=true;window.addEventListener('online',()=>{teQueueFlush().catch(()=>{});if(sb){sb.from('app_data').select('updated_at').eq('id','main').single().then(({data:row})=>{const serverTs=row&&row.updated_at?new Date(row.updated_at).getTime():0;const _lTs=localTs();if(_lTs>serverTs+3000){const local=localLoad();if(local){sb.from('app_data').upsert({id:'main',data:local,updated_at:new Date(_lTs).toISOString()}).then(()=>console.log('Online: local data pushed to Supabase'))}}}).catch(()=>{})}})}
// ========== FIN POINTAGE FIABLE ==========
...
<h3 style={{margin:0,fontSize:16}}>🗺 Carte planning — {selDate} · {todayCount} chantier(s) <span style={{fontSize:10,color:C.dim,fontWeight:400,marginLeft:8}}>v2026.06.03-1</span></h3>
