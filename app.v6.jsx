...
const PLANNING_LAYOUT_DEFAULTS={vue:'tableau',zoom:100,cardGap:12,leftColW:95,cardRadius:10,cardBorder:3,empFont:15,machFont:13,chantierFont:15,locWidth:220,rowGap:5,rowPadV:6,chipFont:13};
...
let _liveData=null; // dernier etat applicatif connu, toujours frais (anti stale-closure: les editeurs clonent depuis ici)
...
// Horodate (_updatedAt) chaque item nouveau ou modifie -> permet au merge "le plus recent gagne" entre appareils
const stampUpdatedAt=(prevArr,nextArr)=>{const pm=new Map((prevArr||[]).filter(x=>x&&x.id).map(x=>[x.id,JSON.stringify(x)]));(nextArr||[]).forEach(it=>{if(!it||!it.id)return;const ps=pm.get(it.id);if(ps===undefined||ps!==JSON.stringify(it))it._updatedAt=Date.now()})};
// Groupes de notifs Telegram coordonnees (Option 1) : on prefere la version marquee "doneBy" (un admin a clique), sinon la plus recente ; purge >24h.
const mergeTgGroups=(local,remote)=>{const now=Date.now();const map=new Map();(remote||[]).forEach(g=>{if(g&&g.id)map.set(g.id,g)});(local||[]).forEach(g=>{if(!g||!g.id)return;const r=map.get(g.id);if(!r){map.set(g.id,g);return}let win;if(g.doneBy&&!r.doneBy)win=g;else if(r.doneBy&&!g.doneBy)win=r;else win=((g.doneAt||g.ts||0)>=(r.doneAt||r.ts||0))?g:r;map.set(g.id,win)});return[...map.values()].filter(g=>g&&now-(g.ts||0)<86400000)};
...
merged.jobs=mergeByUpdatedAt(local.jobs,remote.jobs,ts.jobs);
...
merged.tgGroups=mergeTgGroups(local.tgGroups,remote.tgGroups);
return merged};
...
const _mergedArrays={timeEntries:1,timeEntriesValidated:1,panneReports:1,interventions:1,parts:1,jobs:1,clients:1,depots:1,employees:1,machines:1,trucks:1,cars:1,machineReports:1,jdReports:1,messages:1,maintenanceRequests:1,stations:1,stationProducts:1,stationUsers:1,stationMovements:1,tgGroups:1,_tombstones:1};
...
const teUpsertRemote=async(entry,table)=>{if(!sb)return false;const row=teToRow(entry);for(let essai=0;essai<5;essai++){try{const{error}=await sb.from(table).upsert(row,{onConflict:'id'});if(!error)return true;const col=(error.code==='42703'||error.code==='PGRST204')?teColonneRefusee(error):null;if(col&&Object.prototype.hasOwnProperty.call(row,col)){console.warn('TE upsert : colonne '+col+' absente dans '+table+', ignoree (executer le SQL de SETUP.md)');delete row[col];continue}console.error('TE upsert err',table,error);return false}catch(e){console.warn('TE upsert exc',e);return false}}return false};
...
// Pointage ouvert (embauche sans debauche) cree AVANT un autre pointage du meme salarie le meme jour = double clic pendant une panne
// de synchro (l'embauche avait disparu de l'ecran, le chauffeur a re-embauche). On ne le renvoie pas, sinon il resterait ouvert a jamais.
const teEstDoublonOuvert=(e,liste)=>{if(!e||!e.startTime||e.endTime||e.type==='absence'||e.type==='pending')return false;return liste.some(o=>o&&o.id!==e.id&&o.empId===e.empId&&o.date===e.date&&(o.createdAt||'')>(e.createdAt||''))};
const teOpCle=op=>op.kind+':'+(op.entry?op.entry.id:op.id)+':'+(op._ts||0);
let teFlushEnCours=false;
const teQueueFlush=async()=>{if(!sb||teFlushEnCours)return;const avail=await teTestTables();if(!avail)return;const q=teQueueGet();if(!q.length)return;teFlushEnCours=true;try{
// une meme entree n'est envoyee qu'une fois, dans son dernier etat
const parId=new Map();const aEnvoyer=[];for(const op of q){if(op.kind==='upsert'&&op.entry&&op.entry.id){const k=op.table+':'+op.entry.id;if(parId.has(k))aEnvoyer[parId.get(k)]=null;parId.set(k,aEnvoyer.length)}aEnvoyer.push(op)}
const ups=aEnvoyer.filter(op=>op&&op.kind==='upsert'&&op.table==='time_entries').map(op=>op.entry);
const restantes=new Set();let envoyes=0;
for(const op of aEnvoyer){if(!op)continue;if(op.kind==='upsert'&&op.table==='time_entries'&&teEstDoublonOuvert(op.entry,ups)){console.log('TE queue : embauche en double ignoree',op.entry.date,op.entry.startTime);continue}
let ok=false;if(op.kind==='upsert')ok=await teUpsertRemote(op.entry,op.table);else if(op.kind==='delete')ok=await teDeleteRemote(op.id,op.table);if(ok)envoyes++;else restantes.add(teOpCle(op))}
// on ne retire de la file que ce qu'on a traite : une operation ajoutee pendant l'envoi est conservee
const lues=new Set(q.map(teOpCle));teQueueSet(teQueueGet().filter(op=>!lues.has(teOpCle(op))||restantes.has(teOpCle(op))));
if(envoyes)console.log('TE queue flushed:',envoyes,'envoyes, restantes:',restantes.size)}finally{teFlushEnCours=false}};
const teSyncChanges=async(oldData,newData)=>{if(!sb)return;let avail=false;try{avail=await teTestTables()}catch(e){console.warn('test tables pointages',e)}for(const key of['timeEntries','timeEntriesValidated']){const table=teTableOf(key);const oldMap=new Map(((oldData||{})[key]||[]).filter(t=>t&&t.id).map(t=>[t.id,t]));const nextMap=new Map(((newData||{})[key]||[]).filter(t=>t&&t.id).map(t=>[t.id,t]));for(const[id,entry]of nextMap){const prev=oldMap.get(id);if(!prev||JSON.stringify(prev)!==JSON.stringify(entry)){let ok=false;if(avail){try{ok=await teUpsertRemote(entry,table)}catch(e){console.warn('envoi pointage',e)}}if(!ok)teQueuePush({kind:'upsert',entry,table})}}}
// SUPPRESSION EXPLICITE UNIQUEMENT : on ne deduit PLUS jamais une suppression de l'absence d'un id dans la liste locale.
// (Bug historique : un appareil avec une copie partielle/perimee des pointages -> le save effacait les pointages des AUTRES salaries.)
// Seuls les ids explicitement listes dans newData._teDeletes (poses par delTE / copyFromDeclared) sont supprimes.
const dels=(newData&&newData._teDeletes)||[];for(const dd of dels){if(!dd||!dd.id)continue;const table=teTableOf(dd.key==='timeEntriesValidated'?'timeEntriesValidated':'timeEntries');if(avail){const ok=await teDeleteRemote(dd.id,table);if(!ok)teQueuePush({kind:'delete',id:dd.id,table})}else{teQueuePush({kind:'delete',id:dd.id,table})}}};
const teMigrateFromBlob=async(data)=>{if(!sb||!data)return data;const avail=await teTestTables();if(!avail)return data;const merged={...data};for(const key of['timeEntries','timeEntriesValidated']){const table=teTableOf(key);const remote=await teLoadAll(table);if(remote===null){merged[key]=data[key]||[];continue}const remoteMap=new Map(remote.map(t=>[t.id,t]));let allIds=null;try{const{data:allRows}=await sb.from(table).select('id');if(allRows)allIds=new Set(allRows.map(r=>r.id))}catch(e){}const blobEntries=(data[key]||[]).filter(t=>t&&t.id);const toMigrate=blobEntries.filter(t=>!remoteMap.has(t.id)&&(!allIds||!allIds.has(t.id))&&!(key==='timeEntries'&&teEstDoublonOuvert(t,[...remote,...blobEntries])));if(toMigrate.length){console.log('Migration de',toMigrate.length,key,'vers',table);for(const e of toMigrate){const ok=await teUpsertRemote(e,table);if(ok)remoteMap.set(e.id,e);else teQueuePush({kind:'upsert',entry:e,table})}}merged[key]=[...remoteMap.values()]}return merged};
...
if(typeof window!=='undefined'&&!window.__teOnlineBound){window.__teOnlineBound=true;window.addEventListener('online',()=>{teQueueFlush().catch(()=>{});if(sb){const local=localLoad();if(local){saveData(local).then(()=>console.log('Online: local data merged with Supabase')).catch(()=>{})}}})}
// ========== FIN POINTAGE FIABLE ==========
...
// jszip (98 Ko) ne servent QU'AUX IMPORTS telematiques, cote admin : inutile de les
// faire telecharger a chaque ouverture, surtout aux chauffeurs sur telephone.
const _libs={};
const chargerLib=(url,test)=>{if(test())return Promise.resolve(true);if(_libs[url])return _libs[url];
  _libs[url]=new Promise(res=>{const s=document.createElement('script');s.src=url;s.async=true;
    s.onload=()=>res(test());s.onerror=()=>res(false);document.head.appendChild(s)});return _libs[url]};
const libXLSX=()=>chargerLib('https://cdn.sheetjs.com/xlsx-0.20.3/package/dist/xlsx.full.min.js',()=>!!window.XLSX);
const libJSZip=()=>chargerLib('https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js',()=>!!window.JSZip);
...
// ===== Notifications Telegram (envoyees directement par le navigateur, sans serveur) =====
const tgSend=async(token,chatId,text,extra)=>{if(!token||!chatId)return{ok:false,error:'Token ou ID Telegram manquant'};try{const body={chat_id:chatId,text:text};if(extra)Object.assign(body,extra);const r=await fetch('https://api.telegram.org/bot'+token+'/sendMessage',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const j=await r.json();return j.ok?{ok:true,messageId:j.result&&j.result.message_id}:{ok:false,error:j.description||'Erreur Telegram'}}catch(e){return{ok:false,error:String((e&&e.message)||e)}}};
const tgGetMe=async(token)=>{try{const r=await fetch('https://api.telegram.org/bot'+token+'/getMe');const j=await r.json();return j.ok?{ok:true,username:j.result.username,name:j.result.first_name}:{ok:false,error:j.description||'Token invalide'}}catch(e){return{ok:false,error:String((e&&e.message)||e)}}};
const tgAdminChats=(cfg)=>{const s=new Set();if(cfg&&cfg.telegramAdminChatId)s.add(String(cfg.telegramAdminChatId));((cfg&&cfg.telegramAdminChats)||[]).forEach(a=>{const c=a&&(a.chatId||a);if(c)s.add(String(c))});return[...s]};
const tgSendAdmins=async(cfg,text,extra)=>{const tok=cfg&&cfg.telegramBotToken;if(!tok)return[];const sent=[];for(const cid of tgAdminChats(cfg)){const r=await tgSend(tok,cid,text,extra);if(r&&r.ok&&r.messageId)sent.push({c:String(cid),m:r.messageId})}return sent};
// Detecte tout produit station qui passe a 0 (depuis >0) et alerte les admins. Centralise -> couvre tous les chemins (retrait, transfert, edition).
const _checkStockZero=(prev,nd)=>{try{if(!nd||!nd.telegramBotToken||nd.tgNotifyStock===false)return;const pm=new Map(((prev&&prev.stationProducts)||[]).filter(p=>p&&p.id).map(p=>[p.id,p]));(nd.stationProducts||[]).forEach(p=>{if(!p||!p.id)return;const old=pm.get(p.id);if(old&&Number(old.quantity)>0&&Number(p.quantity)<=0){const st=(nd.stations||[]).find(s=>s.id===p.stationId);const sn=st?st.name:'station inconnue';const sl=st&&st.location?' — '+st.location:'';tgSendAdmins(nd,'📦 STOCK ÉPUISÉ\n'+p.name+(p.unit?' ('+p.unit+')':'')+'\n📍 '+sn+sl)}})}catch(e){}};
const tgGetChatId=async(token)=>{try{const r=await fetch('https://api.telegram.org/bot'+token+'/getUpdates');const j=await r.json();if(!j.ok)return{ok:false,error:j.description||'Token invalide'};const ups=j.result||[];for(let i=ups.length-1;i>=0;i--){const m=ups[i].message||ups[i].edited_message;if(m&&m.chat&&m.chat.id!=null)return{ok:true,chatId:String(m.chat.id),name:((m.chat.first_name||'')+' '+(m.chat.last_name||'')).trim()||(m.chat.username?'@'+m.chat.username:'')}}return{ok:false,error:'Aucun message recu. Dans Telegram : ouvre ton bot, appuie sur Demarrer, envoie un message, puis reessaie.'}}catch(e){return{ok:false,error:String((e&&e.message)||e)}}};
...
// Extrait la largeur numerique d'une raboteuse : priorite m.width puis m.name (ex: '130 CFI2' -> '130', '' + nom 'CFI2 200' -> '200')
const getMachineWidth=m=>{if(!m)return'';const w=String(m.width||'').match(/\d+/);if(w)return w[0];const n=String(m.name||'').match(/\d+/);if(n)return n[0];return''};
const getForfaitKey=(data,cid,machine)=>{const cl=(data.clients||[]).find(x=>x.id===cid);const p=(cl&&cl.forfaitType==='specific')?cid:'standard';if(!machine)return null;if(machine.type==='Raboteuse')return p+'_rab_'+getMachineWidth(machine);if(machine.type==='Balayeuse')return p+'_bal';if(machine.type==='Citerne')return p+'_cit';return null};
const getForfaitPrice=(data,cid,machine,ft,citOpt,isNight)=>{let k=getForfaitKey(data,cid,machine);if(!k)return 0;
// Citerne sans option choisie : defaut 'Avec chauffeur' pour que le calcul fonctionne meme depuis le planning (modal mission permet de changer)
if(machine&&machine.type==='Citerne'){k+='_'+(citOpt||'Avec chauffeur')}
let g=data.forfaits[k];
// Fallback retro-compat : si pas de tarif a la cle normalisee, essayer la cle brute (avant normalisation width) ou autre variante citerne
if(!g&&machine){const cl=(data.clients||[]).find(x=>x.id===cid);const p=(cl&&cl.forfaitType==='specific')?cid:'standard';if(machine.type==='Raboteuse'){g=data.forfaits[p+'_rab_'+(machine.width||'')]}if(!g&&machine.type==='Citerne'){const candKeys=Object.keys(data.forfaits||{}).filter(kk=>kk.startsWith(p+'_cit_'));if(candKeys.length>=1)g=data.forfaits[candKeys[0]]}}
if(!g)return 0;let pr=g[ft]||0;if(isNight)pr=pr*(1+(data.nightPct||30)/100);return Math.round(pr*100)/100};
...
// Pas d'heure = pas d'heure : aucun 08:00 invente nulle part dans l'app (regle du 09/09/2026, planning de papa)
...
const[billStart,setBillStart]=useState(job?(job.billingStart||''):'');
...
{job&&<button onClick={()=>{if(!confirm('Supprimer cette mission ?'))return;const nd=JSON.parse(JSON.stringify(_liveData||data));tombstone(nd,'jobs',job.id);nd.jobs=nd.jobs.filter(j=>j.id!==job.id);save(nd);onClose()}} style={btnStyle(C.red)}>Supprimer</button>}
...
const toggleSent=()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const j=nd.jobs.find(x=>x.id===job.id);if(j){j.sent=!j.sent;save(nd)}};
...
if(!window.JSZip)await libJSZip();
...
// Garde-fou : si la vue tableau plante, on affiche l'erreur et un bouton pour revenir a la vue classique
// (sans ca, une erreur de rendu ferait tomber toute l'app et l'admin ne pourrait plus rien changer).
class GardeVue extends React.Component{constructor(p){super(p);this.state={err:null}}static getDerivedStateFromError(e){return{err:e}}componentDidCatch(e){console.error('Vue tableau',e)}render(){if(this.state.err)return(<div style={{background:'#fee2e2',border:'2px solid #dc2626',borderRadius:8,padding:16,color:'#7f1d1d'}}><b>La vue tableau a rencontré une erreur.</b> {String((this.state.err&&this.state.err.message)||'')}<div style={{marginTop:8}}><button onClick={this.props.onReset} style={{padding:'6px 12px',borderRadius:6,border:'2px solid #dc2626',background:'#fff',color:'#dc2626',fontWeight:700,cursor:'pointer'}}>Revenir à la vue classique</button></div></div>);return this.props.children}}
...
const caType=t=>dayMissions.filter(j=>((data.machines||[]).find(m=>m.id===j.machineId)||{}).type===t).reduce((s,j)=>s+(j.priceForfait||0)+(j.hasTransfer?j.transferPrice||0:0),0);
// Envoi d'un chantier au chauffeur sur Telegram (le meme code que le bouton ✓ de la vue classique)
const envoyerChantier=async(j)=>{const cfg=_liveData||data;const tok=cfg.telegramBotToken;const link=(cfg.telegramEmpChats||{})[j.employeeId];const drv=(cfg.employees||[]).find(e=>e.id===j.employeeId);const drvNm=drv?drv.name:'le chauffeur';if(!tok||!link||!link.chatId){const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.sent=!jj.sent;save(nd)}alert(!tok?'Bot Telegram non configure (Reglages).':drvNm+" n'a pas encore lie son Telegram.");return}const cl2=(cfg.clients||[]).find(c=>c.id===j.clientId);const m2=(cfg.machines||[]).find(x=>x.id===j.machineId);const prenom=drv&&drv.name?drv.name.split(' ')[0]:'';const lines=['👋 Salut '+prenom+' !','📋 Ton chantier — '+fmtDate(new Date(j.date+'T12:00:00'))+' :'];lines.push('• '+(j.billingStart||'')+' '+(j.location||(cl2?cl2.name:'chantier'))+(cl2&&j.location?' ('+cl2.name+')':'')+(m2?' ['+m2.name+']':''));if(j.siteManager)lines.push('👷 Chef : '+j.siteManager+(j.siteManagerPhone?' ('+j.siteManagerPhone+')':''));const co=parseCoords(j.gps||j._geocodedGps);if(co)lines.push('🗺 https://www.google.com/maps?q='+co[0]+','+co[1]);lines.push('','👇 Merci de confirmer que tu as bien lu.');const r=await tgSend(tok,link.chatId,lines.join('\n'),{reply_markup:{inline_keyboard:[[{text:'✅ J ai bien lu',callback_data:'ack:'+j.id}]]}});if(r&&r.ok){const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.sent=true;jj.ack=false;jj._updatedAt=Date.now();save(nd)}}else{alert('❌ '+((r&&r.error)||'Erreur Telegram'))}};
// Zoom automatique de la vue tableau : toute la journee doit tenir sur l'ecran (hauteur ET largeur), sans defiler.
...
// Telephone (24/09/2026) : une seule colonne, cartes empilees, chaque chantier sur deux lignes, pas de zoom
const[mob,setMob]=useState(()=>window.innerWidth<700);useEffect(()=>{const f=()=>setMob(window.innerWidth<700);window.addEventListener('resize',f);return()=>window.removeEventListener('resize',f)},[]);
...
useEffect(()=>{if(PL.vue==='classique')return;const calc=()=>{const el=fitRef.current;if(!el)return;el.style.zoom='1';if(mob)return;const top=el.getBoundingClientRect().top;const availH=window.innerHeight-top-10;const availW=(el.parentElement?el.parentElement.clientWidth:el.clientWidth)||el.clientWidth;const natH=el.scrollHeight,natW=el.scrollWidth;let z=1;if(natH>0&&availH>0)z=Math.min(z,availH/natH);if(natW>0&&availW>0)z=Math.min(z,availW/natW);z=Math.max(0.4,Math.min(1,z));el.style.zoom=String(Math.round(z*1000)/1000)};const t=setTimeout(calc,0);window.addEventListener('resize',calc);return()=>{clearTimeout(t);window.removeEventListener('resize',calc)}},[data.jobs,data.machines,data.employees,selDate,PL.vue,sbHidden,mob,JSON.stringify(PL.tabFont||null)]);
const withJobs=[...new Set(dayJobs.map(j=>j.employeeId))];
const availDrivers=(data.employees||[]).filter(e=>!withJobs.includes(e.id));
const saveDepotJob=()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.jobs)nd.jobs=[];nd.jobs.push({id:uid(),date:selDate,employeeId:depotFormEmpId,type:'depot',depotId:depotFormDepotId,depotActivity:depotFormActivity,depotDescription:depotFormDesc});save(nd);setShowDepotForm(false);setDepotFormEmpId('');setDepotFormDepotId('');setDepotFormActivity(DEPOT_ACTIVITIES[0]);setDepotFormDesc('')};
...
if(umJobs.length===0){
const createUmJob=(field,value)=>{const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.jobs)nd.jobs=[];const newJ={id:uid(),date:selDate,employeeId:'',machineId:um.id,clientId:'',agencyName:'',siteManager:'',siteManagerPhone:'',location:'',gps:'',forfaitType:'',priceForfait:0,isNight:false,hasTransfer:false,transferPrice:0,billingStart:'',startFrom:'',endAt:'',machineFuelL:0,machineFuelDepot:'',kmAller:0,kmRetour:0,travelMinAller:0,travelMinRetour:0,distanceKm:0,travelMin:0,sent:false};newJ[field]=value;nd.jobs.push(newJ);save(nd)};
...
<select style={{fontSize:13,fontWeight:700,border:'1px solid '+C.border,borderRadius:6,padding:'3px 4px',background:'#fff',width:'100%',textAlign:'center'}} value="" onChange={e2=>{if(!e2.target.value)return;const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.jobs)nd.jobs=[];nd.jobs.push({id:uid(),date:selDate,employeeId:e2.target.value,machineId:um.id,clientId:'',agencyName:'',siteManager:'',siteManagerPhone:'',location:'',gps:'',forfaitType:'',priceForfait:0,isNight:false,hasTransfer:false,transferPrice:0,billingStart:'',startFrom:'',endAt:'',machineFuelL:0,machineFuelDepot:'',kmAller:0,kmRetour:0,travelMinAller:0,travelMinRetour:0,distanceKm:0,travelMin:0,sent:false});save(nd)}}><option value="">Chauff.</option>{(data.employees||[]).map(e2=><option key={e2.id} value={e2.id}>{e2.name}</option>)}</select>
...
<button onClick={()=>createUmJob('billingStart','')} title="Ajouter un chantier sans chauffeur" style={{background:umColor,color:'#fff',border:'none',borderRadius:6,width:28,height:28,cursor:'pointer',fontSize:18,fontWeight:700,lineHeight:1,padding:0,flexShrink:0}}>+</button>
</div>
</div>
</div></React.Fragment>)}
...
<select style={{fontSize:13,fontWeight:700,border:'1px solid '+C.border,borderRadius:6,padding:'3px 4px',background:'#fff',width:'100%',textAlign:'center'}} value="" onChange={e2=>{if(!e2.target.value)return;const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.jobs)nd.jobs=[];const existingJ=nd.jobs.filter(x=>x.machineId===um.id&&x.date===selDate);if(existingJ.length>0){existingJ.forEach(x2=>{x2.employeeId=e2.target.value})}else{nd.jobs.push({id:uid(),date:selDate,employeeId:e2.target.value,machineId:um.id,clientId:'',agencyName:'',siteManager:'',siteManagerPhone:'',location:'',gps:'',forfaitType:'',priceForfait:0,isNight:false,hasTransfer:false,transferPrice:0,billingStart:'',startFrom:'',endAt:'',machineFuelL:0,machineFuelDepot:'',kmAller:0,kmRetour:0,travelMinAller:0,travelMinRetour:0,distanceKm:0,travelMin:0,sent:false})}save(nd)}}><option value="">Chauff.</option>{(data.employees||[]).map(e2=><option key={e2.id} value={e2.id}>{e2.name}</option>)}</select>
<div style={{fontSize:13,fontWeight:700,color:umColor,textAlign:'center'}}>{um.name}{um.width?' ('+um.width+')':''}</div>
<button onClick={e=>{e.stopPropagation();const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.jobs)nd.jobs=[];nd.jobs.push({id:uid(),date:selDate,employeeId:'',machineId:um.id,clientId:'',agencyName:'',siteManager:'',siteManagerPhone:'',location:'',gps:'',forfaitType:'',priceForfait:0,isNight:false,hasTransfer:false,transferPrice:0,billingStart:'',startFrom:'',endAt:'',machineFuelL:0,machineFuelDepot:'',kmAller:0,kmRetour:0,travelMinAller:0,travelMinRetour:0,distanceKm:0,travelMin:0,sent:false});save(nd)}} style={{background:C.accent,color:'#fff',border:'none',borderRadius:4,width:22,height:22,cursor:'pointer',fontSize:14,fontWeight:700,lineHeight:'20px',padding:0}}>+</button>
</div>
<div style={{flex:1,minWidth:0}}>
...
<div style={{padding:PL.rowPadV+'px 10px',display:'flex',alignItems:'center',gap:PL.rowGap,flexWrap:'wrap'}}>
<input key={'ujcl_'+uj.id+'_'+(uj.clientId||'_')} list="planning-clients-list" placeholder="Client" defaultValue={ujCl?ujCl.name:''} onChange={e=>{const v=e.target.value;const matched=(data.clients||[]).find(c2=>c2.name===v);if(!matched)return;const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===uj.id);if(!jj)return;jj.clientId=matched.id;if(ujM&&jj.forfaitType){const p=getForfaitPrice(nd,matched.id,ujM,jj.forfaitType,jj.citOption,jj.isNight);if(p)jj.priceForfait=p}save(nd)}} onBlur={e=>{const v=e.target.value.trim();const exists=(data.clients||[]).find(c2=>c2.name.toLowerCase()===v.toLowerCase());if(exists){if(uj.clientId!==exists.id){const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===uj.id);if(jj){jj.clientId=exists.id;if(ujM&&jj.forfaitType){const p=getForfaitPrice(nd,exists.id,ujM,jj.forfaitType,jj.citOption,jj.isNight);if(p)jj.priceForfait=p}save(nd)}}return}if(!v){if(uj.clientId){const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===uj.id);if(jj){jj.clientId='';save(nd)}}return}const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.clients)nd.clients=[];const nc={id:uid(),name:v,forfaitType:'standard',agencies:[],siteManagers:[]};nd.clients.push(nc);const jj=nd.jobs.find(x=>x.id===uj.id);if(jj)jj.clientId=nc.id;save(nd)}} style={{fontSize:15,padding:'4px 6px',borderRadius:6,border:'1px solid '+C.border,background:'#fff',minWidth:100,maxWidth:150}}/>
{ujCl&&(ujCl.agencies||[]).length>0&&<select value={uj.agencyName||''} onChange={e=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===uj.id);if(jj){jj.agencyName=e.target.value;save(nd)}}} title={uj.agencyName?'Agence : '+uj.agencyName:'Choisir une agence'} style={{fontSize:13,padding:'4px 4px',borderRadius:6,border:'1px solid '+C.border,background:uj.agencyName?'#eff6ff':'#fff',color:uj.agencyName?'#1d4ed8':C.dim,fontWeight:uj.agencyName?700:400,width:80,maxWidth:90,flexShrink:0}}><option value="">Agence</option>{(ujCl.agencies||[]).map((a,i)=><option key={i} value={a}>{a}</option>)}</select>}
<select value={uj.siteManager||''} onChange={e=>{if(e.target.value==='__new__'){const n=prompt('Nouveau chef'+(uj.agencyName?' (agence : '+uj.agencyName+')':'')+' :');if(n){const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===uj.id);if(jj){jj.siteManager=n;const cl2=(nd.clients||[]).find(c2=>c2.id===uj.clientId);if(cl2){if(!cl2.siteManagers)cl2.siteManagers=[];if(!cl2.siteManagers.find(s=>s.name===n&&(s.agency||'')===(uj.agencyName||''))){const ph=prompt('Tel (optionnel):','')||'';cl2.siteManagers.push({name:n,phone:ph,agency:uj.agencyName||''});jj.siteManagerPhone=ph}}save(nd)}}}else{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===uj.id);if(jj){jj.siteManager=e.target.value;const cl2=(data.clients||[]).find(c2=>c2.id===uj.clientId);const sm=(cl2&&cl2.siteManagers||[]).find(s=>s.name===e.target.value&&(!uj.agencyName||!s.agency||s.agency===uj.agencyName));if(sm)jj.siteManagerPhone=sm.phone||'';save(nd)}}}} style={{fontSize:15,padding:'4px 6px',borderRadius:6,border:'1px solid '+C.border,background:'#fff',minWidth:80,maxWidth:130}}>
...
<input value={uj.location||''} placeholder="Lieu / adresse" onChange={e=>{const v=e.target.value;const tgt=e.target;const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===uj.id);if(jj){jj.location=v;if(jj._geocodedGps)jj._geocodedGps='';save(nd)}triggerAutoGeocode(uj.id,v,tgt)}} onFocus={e=>{if(uj.location&&uj.location.trim().length>=3&&!uj._geocodedGps)triggerAutoGeocode(uj.id,uj.location,e.target)}} style={{fontSize:PL.chantierFont,padding:'4px 8px',borderRadius:6,border:'1px solid '+(uj._geocodedGps?C.green:C.border),minWidth:100,flex:1,maxWidth:PL.locWidth,background:uj._geocodedGps?'#dcfce7':'#fff'}}/>
<input type="time" value={uj.billingStart||''} onChange={e=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===uj.id);if(jj){jj.billingStart=e.target.value||'';save(nd)}}} style={{fontSize:15,padding:'4px 4px',borderRadius:6,border:'2px solid '+C.orange+'40',background:C.orange+'08',color:C.orange,fontWeight:700,width:75}}/>
...
if(gpsJ&&!uj.startFrom&&shortDep){setTimeout(()=>{const nd2=JSON.parse(JSON.stringify(_liveData||data));const jj2=nd2.jobs.find(x=>x.id===uj.id);if(jj2&&!jj2.startFrom){jj2.startFrom=shortDep.id;jj2.kmAller=shortDep.km;jj2.travelMinAller=Math.round((shortDep.km/80)*60);if(!jj2.endAt&&shortArr){jj2.endAt=shortArr.id;jj2.kmRetour=shortArr.km;jj2.travelMinRetour=Math.round((shortArr.km/80)*60)}jj2.distanceKm=(jj2.kmAller||0)+(jj2.kmRetour||0);jj2.travelMin=(jj2.travelMinAller||0)+(jj2.travelMinRetour||0);save(nd2)}},0)}
return(<React.Fragment>
<select value={uj.startFrom||''} onChange={e=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===uj.id);if(jj){jj.startFrom=e.target.value;const sel2=depOptions2.find(o=>o.id===e.target.value);jj.kmAller=sel2&&sel2.km?sel2.km:0;jj.travelMinAller=sel2&&sel2.km?Math.round((sel2.km/80)*60):0;jj.distanceKm=(jj.kmAller||0)+(jj.kmRetour||0);jj.travelMin=(jj.travelMinAller||0)+(jj.travelMinRetour||0);save(nd)}}} style={{fontSize:12,padding:'2px 3px',borderRadius:4,border:'1px solid #0891b240',background:uj.startFrom&&shortDep&&uj.startFrom===shortDep.id?'#0891b218':'#0891b208',color:'#0891b2',fontWeight:600,minWidth:60,maxWidth:110}}>
...
<button onClick={()=>{if(confirm('Supprimer ?')){const nd=JSON.parse(JSON.stringify(_liveData||data));tombstone(nd,'jobs',uj.id);nd.jobs=nd.jobs.filter(x=>x.id!==uj.id);save(nd)}}} style={{background:'none',border:'none',cursor:'pointer',fontSize:16,color:C.red}}>×</button>
</div>
</div>
</div>)})}
</div>
...
{depotJobs.map(dj=>{const dep=getDepot(dj.depotId);const isRest=dj.rest||dj.depotActivity==='Repos';return(
<div key={dj.id} style={{background:isRest?'#fee2e2':'#f8fafc',borderRadius:8,marginBottom:8,border:isRest?'2px solid '+C.red:'1px solid '+C.border,borderLeft:isRest?'6px solid '+C.red:'4px solid #64748b',padding:'8px 12px'}}>
<div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
<span style={{fontSize:15,fontWeight:700,color:isRest?C.red:'#64748b'}}>{isRest?'\u{1F634} '+emp.name+' — REPOS':<>&#127959; {emp.name} — {dep?dep.name:'Depot'} — {dj.depotActivity||'Depot'}</>}</span>
{!isRest&&dj.depotDescription&&<span style={{fontSize:14,color:C.dim}}>({dj.depotDescription})</span>}
<button onClick={()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));tombstone(nd,'jobs',dj.id);nd.jobs=nd.jobs.filter(x=>x.id!==dj.id);save(nd)}} style={{marginLeft:'auto',background:'none',border:'none',cursor:'pointer',fontSize:16,color:C.red}}>x</button>
</div>
{!isRest&&<div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',marginTop:4,fontSize:13}}>
...
const createJobForEmp=(field,value)=>{const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.jobs)nd.jobs=[];const newJ={id:uid(),date:selDate,employeeId:eId,machineId:emp.machineId||'',clientId:'',agencyName:'',siteManager:'',siteManagerPhone:'',location:'',gps:'',forfaitType:'',priceForfait:0,isNight:false,hasTransfer:false,transferPrice:0,billingStart:'',startFrom:'',endAt:'',machineFuelL:0,machineFuelDepot:'',kmAller:0,kmRetour:0,travelMinAller:0,travelMinRetour:0,distanceKm:0,travelMin:0,sent:false};newJ[field]=value;nd.jobs.push(newJ);save(nd)};
return(
...
<div style={{flex:1,minWidth:0}}>
<div style={{padding:PL.rowPadV+'px 10px',display:'flex',alignItems:'center',gap:PL.rowGap,flexWrap:'wrap',opacity:0.5}}>
<input list="planning-clients-list" placeholder="Client" onChange={e=>{const v=e.target.value;const matched=(data.clients||[]).find(c2=>c2.name===v);if(matched){createJobForEmp('clientId',matched.id);e.target.value=''}}} onBlur={e=>{const v=e.target.value.trim();if(!v)return;const matched=(data.clients||[]).find(c2=>c2.name.toLowerCase()===v.toLowerCase());if(matched){createJobForEmp('clientId',matched.id);e.target.value='';return}const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.clients)nd.clients=[];const nc={id:uid(),name:v,forfaitType:'standard',agencies:[],siteManagers:[]};nd.clients.push(nc);if(!nd.jobs)nd.jobs=[];nd.jobs.push({id:uid(),date:selDate,employeeId:eId,machineId:emp.machineId||'',clientId:nc.id,agencyName:'',siteManager:'',siteManagerPhone:'',location:'',gps:'',forfaitType:'',priceForfait:0,isNight:false,hasTransfer:false,transferPrice:0,billingStart:'',startFrom:'',endAt:'',machineFuelL:0,machineFuelDepot:'',kmAller:0,kmRetour:0,travelMinAller:0,travelMinRetour:0,distanceKm:0,travelMin:0,sent:false});save(nd);e.target.value=''}} style={{fontSize:15,padding:'4px 6px',borderRadius:6,border:'1px solid '+C.border,background:'#fff',minWidth:100,maxWidth:150}}/>
...
<input placeholder="Lieu / adresse" onKeyDown={e=>{if(e.key==='Enter'&&e.target.value){createJobForEmp('location',e.target.value);e.target.value=''}}} style={{fontSize:PL.chantierFont,padding:'4px 8px',borderRadius:6,border:'1px solid '+C.border,minWidth:100,flex:1,maxWidth:PL.locWidth,background:'#fff'}}/>
<input type="time" disabled value="" style={{fontSize:15,padding:'4px 4px',borderRadius:6,border:'2px solid '+C.orange+'40',background:C.orange+'08',color:C.orange,fontWeight:700,width:75,opacity:0.5}}/>
...
<button onClick={()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.jobs)nd.jobs=[];nd.jobs.push({id:uid(),date:selDate,employeeId:eId,type:'depot',depotActivity:'Repos',rest:true});save(nd)}} style={{background:C.red,color:'#fff',border:'none',borderRadius:4,padding:'4px 10px',cursor:'pointer',fontSize:13}}>Repos</button>
</div>
...
<div style={{padding:PL.rowPadV+'px 10px',display:'flex',alignItems:'center',gap:PL.rowGap,flexWrap:'wrap'}}>
<button onClick={()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.paid=!jj.paid;save(nd)}}} title={j.paid?'Client a paye':'Marquer client paye'} style={{width:24,height:24,borderRadius:6,border:'2px solid '+(j.paid?C.purple:C.muted),background:j.paid?C.purple:'transparent',color:'#fff',cursor:'pointer',fontSize:13,fontWeight:800,padding:0,lineHeight:1,flexShrink:0}}>{j.paid?'€':''}</button>
<input key={'jcl_'+j.id+'_'+(j.clientId||'_')} list="planning-clients-list" placeholder="Client" defaultValue={cl?cl.name:''} onChange={e=>{const v=e.target.value;const matched=(data.clients||[]).find(c2=>c2.name===v);if(!matched)return;const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(!jj)return;jj.clientId=matched.id;const m3=getMach(jj.machineId);if(m3&&jj.forfaitType){const p=getForfaitPrice(nd,matched.id,m3,jj.forfaitType,jj.citOption,jj.isNight);if(p)jj.priceForfait=p}save(nd)}} onBlur={e=>{const v=e.target.value.trim();const exists=(data.clients||[]).find(c2=>c2.name.toLowerCase()===v.toLowerCase());if(exists){if(j.clientId!==exists.id){const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.clientId=exists.id;const m3=getMach(jj.machineId);if(m3&&jj.forfaitType){const p=getForfaitPrice(nd,exists.id,m3,jj.forfaitType,jj.citOption,jj.isNight);if(p)jj.priceForfait=p}save(nd)}}return}if(!v){if(j.clientId){const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.clientId='';save(nd)}}return}const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.clients)nd.clients=[];const nc={id:uid(),name:v,forfaitType:'standard',agencies:[],siteManagers:[]};nd.clients.push(nc);const jj=nd.jobs.find(x=>x.id===j.id);if(jj)jj.clientId=nc.id;save(nd)}} style={{fontSize:15,padding:'4px 6px',borderRadius:6,border:'2px solid '+(j.paid?C.purple:C.border),background:j.paid?C.purple+'20':'#fff',color:j.paid?C.purple:'inherit',fontWeight:j.paid?700:400,minWidth:100,maxWidth:150}}/>
{cl&&(cl.agencies||[]).length>0&&<select value={j.agencyName||''} onChange={e=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.agencyName=e.target.value;save(nd)}}} title={j.agencyName?'Agence : '+j.agencyName:'Choisir une agence'} style={{fontSize:13,padding:'4px 4px',borderRadius:6,border:'1px solid '+C.border,background:j.agencyName?'#eff6ff':'#fff',color:j.agencyName?'#1d4ed8':C.dim,fontWeight:j.agencyName?700:400,width:80,maxWidth:90,flexShrink:0}}><option value="">Agence</option>{(cl.agencies||[]).map((a,i)=><option key={i} value={a}>{a}</option>)}</select>}
<select value={j.siteManager||''} onChange={e=>{if(e.target.value==='__new__'){const n=prompt('Nouveau chef chantier'+(j.agencyName?' (agence : '+j.agencyName+')':'')+' :');if(n){const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.siteManager=n;const cl2=(nd.clients||[]).find(c2=>c2.id===j.clientId);if(cl2){if(!cl2.siteManagers)cl2.siteManagers=[];if(!cl2.siteManagers.find(s=>s.name===n&&(s.agency||'')===(j.agencyName||''))){const ph=prompt('Tel du chef (optionnel):','')||'';cl2.siteManagers.push({name:n,phone:ph,agency:j.agencyName||''});jj.siteManagerPhone=ph}}save(nd)}}}else{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.siteManager=e.target.value;const cl2=(data.clients||[]).find(c2=>c2.id===j.clientId);const sm=(cl2&&cl2.siteManagers||[]).find(s=>s.name===e.target.value&&(!j.agencyName||!s.agency||s.agency===j.agencyName));if(sm)jj.siteManagerPhone=sm.phone||'';save(nd)}}}} style={{fontSize:15,padding:'4px 6px',borderRadius:6,border:'1px solid '+C.border,background:'#fff',minWidth:80,maxWidth:130}}>
...
<input value={j.location||''} placeholder="Lieu / adresse" onChange={e=>{const v=e.target.value;const tgt=e.target;const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.location=v;if(jj._geocodedGps)jj._geocodedGps='';save(nd)}triggerAutoGeocode(j.id,v,tgt)}} onFocus={e=>{if(j.location&&j.location.trim().length>=3&&!j._geocodedGps)triggerAutoGeocode(j.id,j.location,e.target)}} style={{fontSize:PL.chantierFont,padding:'4px 8px',borderRadius:6,border:'1px solid '+(j._geocodedGps?C.green:C.border),minWidth:100,flex:1,maxWidth:PL.locWidth,background:j._geocodedGps?'#dcfce7':'#fff'}}/>
<input value={j.gps||''} placeholder="📍" title={j.gps?'GPS : '+j.gps+' (cliquer pour modifier)':'Coller le GPS ici (ex: 45.7128,-0.6234)'} onChange={e=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.gps=e.target.value;save(nd)}}} style={{fontSize:13,padding:'4px 4px',borderRadius:6,border:'2px solid '+(j.gps?C.green:C.border),background:j.gps?'#dcfce7':'#fff',width:42,flexShrink:0,textAlign:'center'}}/>
<input type="time" value={j.billingStart||''} onChange={e=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.billingStart=e.target.value||'';save(nd)}}} style={{fontSize:15,padding:'4px 4px',borderRadius:6,border:'2px solid '+C.orange+'40',background:C.orange+'08',color:C.orange,fontWeight:700,width:75}}/>
{/* temps passe sur le chantier : rempli automatiquement a la fin de chantier */}
{(()=>{const sg=j.signature;const dm=sg?sg.durationMin:null;const fini=dm!=null;
 const pd=(sg&&sg.pauseDeducted)||0;
 const titre=fini?('Fin de chantier'+(sg.signedBy?' par '+sg.signedBy:'')+(sg.signedAt?' a '+new Date(sg.signedAt).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'}):'')+(pd?' — pause de '+pd+' min deduite':'')):'Chantier pas encore termine';
 return(<div title={titre} style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
   padding:'2px 9px',borderRadius:8,lineHeight:1.05,flexShrink:0,minWidth:56,
   background:fini?'#dcfce7':'#f1f5f9',border:'2px solid '+(fini?'#86efac':C.border)}}>
  <span style={{fontSize:9,fontWeight:800,textTransform:'uppercase',letterSpacing:'.06em',color:fini?'#166534':C.muted}}>passe</span>
  <b style={{fontSize:15,fontVariantNumeric:'tabular-nums',color:fini?'#166534':C.muted}}>{fini?(Math.floor(dm/60)+'h'+String(dm%60).padStart(2,'0')):'—'}</b>
  </div>);})()}
...
setTimeout(()=>{const nd2=JSON.parse(JSON.stringify(_liveData||data));const jj2=nd2.jobs.find(x=>x.id===j.id);if(!jj2)return;const empJobs=(nd2.jobs||[]).filter(jb=>jb.employeeId===jj2.employeeId&&jb.date===jj2.date&&jb.type!=='depot').sort((a,b)=>(a.billingStart||'99:99').localeCompare(b.billingStart||'99:99'));const myIdx=empJobs.findIndex(jb=>jb.id===jj2.id);const prevJob=myIdx>0?empJobs[myIdx-1]:null;const nextJob=myIdx>=0&&myIdx<empJobs.length-1?empJobs[myIdx+1]:null;const depotIds=new Set(['home',...(nd2.depots||[]).map(d=>d.id)]);const isDepotStart=!jj2.startFrom||depotIds.has(jj2.startFrom);const isDepotEnd=!jj2.endAt||depotIds.has(jj2.endAt);let changed=false;if(isDepotStart||(prevJob&&jj2.startFrom===prevJob.id)){const prevGps=prevJob?parseCoords(prevJob.gps||prevJob._geocodedGps):null;if(prevGps){const km=+(haversine(prevGps,gpsJ)*1.3*0.5).toFixed(0);if(jj2.startFrom!==prevJob.id||Math.abs((jj2.kmAller||0)-km)>=1){jj2.startFrom=prevJob.id;jj2.kmAller=km;jj2.travelMinAller=Math.round((km/80)*60);changed=true}}else if(isDepotStart&&!jj2.startFrom&&shortDep){jj2.startFrom=shortDep.id;jj2.kmAller=shortDep.km;jj2.travelMinAller=Math.round((shortDep.km/80)*60);changed=true}}if(isDepotEnd||(nextJob&&jj2.endAt===nextJob.id)){const nextGps=nextJob?parseCoords(nextJob.gps||nextJob._geocodedGps):null;if(nextGps){const km=+(haversine(gpsJ,nextGps)*1.3*0.5).toFixed(0);if(jj2.endAt!==nextJob.id||Math.abs((jj2.kmRetour||0)-km)>=1){jj2.endAt=nextJob.id;jj2.kmRetour=km;jj2.travelMinRetour=Math.round((km/80)*60);changed=true}}else if(isDepotEnd&&!jj2.endAt&&shortArr){jj2.endAt=shortArr.id;jj2.kmRetour=shortArr.km;jj2.travelMinRetour=Math.round((shortArr.km/80)*60);changed=true}}if(changed){jj2.distanceKm=(jj2.kmAller||0)+(jj2.kmRetour||0);jj2.travelMin=(jj2.travelMinAller||0)+(jj2.travelMinRetour||0);save(nd2)}},0)
return null;
})()}
<button onClick={async()=>{const cfg=_liveData||data;const tok=cfg.telegramBotToken;const link=(cfg.telegramEmpChats||{})[j.employeeId];const drv=(cfg.employees||[]).find(e=>e.id===j.employeeId);const drvNm=drv?drv.name:'le chauffeur';if(!tok||!link||!link.chatId){const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.sent=!jj.sent;save(nd)}alert(!tok?'Bot Telegram non configure (Reglages).':drvNm+" n'a pas encore lie son Telegram.");return}const cl2=(cfg.clients||[]).find(c=>c.id===j.clientId);const m2=(cfg.machines||[]).find(x=>x.id===j.machineId);const prenom=drv&&drv.name?drv.name.split(' ')[0]:'';const lines=['👋 Salut '+prenom+' !','📋 Ton chantier — '+fmtDate(new Date(j.date+'T12:00:00'))+' :'];lines.push('• '+(j.billingStart||'')+' '+(j.location||(cl2?cl2.name:'chantier'))+(cl2&&j.location?' ('+cl2.name+')':'')+(m2?' ['+m2.name+']':''));if(j.siteManager)lines.push('👷 Chef : '+j.siteManager+(j.siteManagerPhone?' ('+j.siteManagerPhone+')':''));const co=parseCoords(j.gps||j._geocodedGps);if(co)lines.push('🗺 https://www.google.com/maps?q='+co[0]+','+co[1]);lines.push('','👇 Merci de confirmer que tu as bien lu.');const r=await tgSend(tok,link.chatId,lines.join('\n'),{reply_markup:{inline_keyboard:[[{text:'✅ J ai bien lu',callback_data:'ack:'+j.id}]]}});if(r&&r.ok){const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.sent=true;jj.ack=false;jj._updatedAt=Date.now();save(nd)}}else{alert('❌ '+((r&&r.error)||'Erreur Telegram'))}}} title={j.sent?'Chantier envoye au chauffeur (Telegram) — recliquer pour renvoyer':'Envoyer le chantier au chauffeur sur Telegram'} style={{width:24,height:24,borderRadius:6,border:'2px solid '+(j.sent?C.green:C.muted),background:j.sent?C.green:'transparent',color:'#fff',cursor:'pointer',fontSize:14,fontWeight:800,padding:0,lineHeight:1,flexShrink:0}}>{j.sent?'✓':''}</button>
<div style={{marginLeft:'auto',display:'flex',gap:4,alignItems:'center'}}>
<button onClick={()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.invoiced=!jj.invoiced;save(nd)}}} title={j.invoiced?'Facture envoyee':'Marquer facture envoyee'} style={{width:24,height:24,borderRadius:6,border:'2px solid '+(j.invoiced?'#eab308':C.muted),background:j.invoiced?'#eab308':'transparent',color:'#fff',cursor:'pointer',fontSize:14,fontWeight:800,padding:0,lineHeight:1,flexShrink:0}}>{j.invoiced?'✓':''}</button>
<button onClick={()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj&&m){jj.hasTransfer=!jj.hasTransfer;if(jj.hasTransfer&&!jj.transferPrice){const tp=getTransferPrice(nd,j.clientId,m,j.citOption,j.isNight);jj.transferPrice=tp||0}save(nd)}}} style={{padding:'4px 8px',borderRadius:6,fontSize:14,border:'2px solid '+(j.invoiced&&j.hasTransfer?'#eab308':j.hasTransfer?C.purple:C.muted),background:j.invoiced&&j.hasTransfer?'#fef9c3':j.hasTransfer?C.purple+'20':'transparent',color:j.invoiced&&j.hasTransfer?'#713f12':j.hasTransfer?C.purple:C.dim,cursor:'pointer',fontWeight:j.hasTransfer?700:400}}>{j.hasTransfer?'T ✓':'+T'}</button>
<select value={j.forfaitType||''} onChange={e=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj&&m){jj.forfaitType=e.target.value;const p=getForfaitPrice(nd,j.clientId,m,e.target.value,j.citOption,j.isNight);if(p)jj.priceForfait=p;else if(e.target.value){let k=getForfaitKey(nd,j.clientId,m)||'?';if(m.type==='Citerne')k+='_'+(j.citOption||'Avec chauffeur');const cn=((nd.clients||[]).find(c=>c.id===j.clientId)||{}).name||'(pas de client)';setTimeout(()=>alert('⚠ Aucun tarif configure\n\nMachine : '+m.name+(m.type==='Raboteuse'?' (largeur extraite : '+(getMachineWidth(m)||'?')+')':'')+'\nForfait : '+e.target.value+'\nClient : '+cn+'\n\nCle recherchee : '+k+'\n\nVa dans Forfaits pour saisir le tarif.'),50)}save(nd)}}} title="Forfait" style={{fontSize:15,padding:'4px 6px',borderRadius:6,border:'2px solid '+(j.invoiced?'#eab308':j.forfaitType?FC[j.forfaitType]||C.accent:C.border),background:j.invoiced?'#fef9c3':j.forfaitType?(FC[j.forfaitType]||C.accent)+'15':'#fff',color:j.invoiced?'#713f12':j.forfaitType?FC[j.forfaitType]||C.accent:C.dim,fontWeight:j.forfaitType?700:400,minWidth:50}}>
...
<button onClick={e=>{e.stopPropagation();if(confirm('Supprimer ?')){const nd=JSON.parse(JSON.stringify(_liveData||data));tombstone(nd,'jobs',j.id);nd.jobs=nd.jobs.filter(x=>x.id!==j.id);save(nd)}}} style={{background:'none',border:'none',cursor:'pointer',fontSize:18,color:C.red,fontWeight:700}}>×</button>
</div>
</div>
...
{/* Boutons d'actions secondaires (deplaces ici depuis la ligne chantier) */}
<div style={{display:'flex',gap:6,alignItems:'center',flexWrap:'wrap',marginBottom:8,padding:'6px 8px',background:'#fff',border:'1px solid '+C.border,borderRadius:8}}>
<div style={{position:'relative'}}><button onClick={e=>{e.stopPropagation();setAddEmpOpen(addEmpOpen===j.id?null:j.id)}} title="Dupliquer ce chantier sur un autre chauffeur" style={{padding:'4px 12px',borderRadius:6,fontSize:14,border:'2px solid '+C.cyan,background:addEmpOpen===j.id?C.cyan+'22':'transparent',color:C.cyan,cursor:'pointer',fontWeight:700,lineHeight:1}}>+ Chauffeur</button>{addEmpOpen===j.id&&<div onClick={e=>e.stopPropagation()} style={{position:'absolute',top:'110%',left:0,zIndex:999,background:'#fff',border:'1px solid '+C.border,borderRadius:8,boxShadow:'0 4px 20px #0003',minWidth:160,overflow:'hidden'}}>{(data.employees||[]).filter(e2=>e2.id!==j.employeeId).map(e2=><div key={e2.id} onClick={()=>{const selEmp2=(data.employees||[]).find(x=>x.id===e2.id);const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.jobs)nd.jobs=[];const nj={...JSON.parse(JSON.stringify(j)),id:uid(),employeeId:e2.id,machineId:selEmp2?selEmp2.machineId||j.machineId:j.machineId,sent:false,ack:false};nd.jobs.push(nj);save(nd);setAddEmpOpen(null)}} style={{padding:'9px 14px',cursor:'pointer',fontSize:14,borderBottom:'1px solid #f1f5f9',color:C.text,userSelect:'none'}} onMouseEnter={e=>e.currentTarget.style.background='#f0f9ff'} onMouseLeave={e=>e.currentTarget.style.background=''}>{e2.name}</div>)}</div>}</div>
<button onClick={()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.isNight=!jj.isNight;if(jj.priceForfait&&jj.forfaitType&&m){const p=getForfaitPrice(nd,jj.clientId,m,jj.forfaitType,jj.citOption,jj.isNight);if(p)jj.priceForfait=p}save(nd)}}} title={j.isNight?'Chantier de nuit (cliquer pour annuler)':'Marquer comme chantier de nuit'} style={{padding:'4px 12px',borderRadius:6,fontSize:13,border:'2px solid '+(j.isNight?C.red:C.muted),background:j.isNight?C.red+'20':'transparent',color:j.isNight?C.red:C.dim,cursor:'pointer',fontWeight:j.isNight?700:600}}>🌙 Nuit</button>
<button onClick={()=>{setDupJobId(j.id);setDupDays(1)}} style={{background:'none',border:'2px solid #d97706',borderRadius:6,fontSize:13,cursor:'pointer',padding:'4px 12px',color:'#d97706',fontWeight:600}} title="Dupliquer sur plusieurs jours">D Dupliquer</button>
<button onClick={e=>{e.stopPropagation();setWirtgenTargetMach(grp.m?grp.m.name:'');if(wirtgenRef.current)wirtgenRef.current.click()}} title="Importer ZIP Wirtgen pour ce chantier" style={{background:'#fff7ed',color:'#9a3412',border:'2px solid #f97316',borderRadius:6,padding:'4px 12px',cursor:'pointer',fontSize:13,fontWeight:700}}>📥 Import Wirtgen</button>
<button onClick={e=>{e.stopPropagation();const mNorm=s=>String(s||'').toUpperCase().replace(/[\s\-_]/g,'');const machName=m?m.name:(grp.m?grp.m.name:'');const mr=(data.machineReports||[]).find(r=>mNorm(r.machineName)===mNorm(machName)&&r.date===j.date);const emp2=(data.employees||[]).find(x=>x.id===j.employeeId);const exp={exportedAt:new Date().toISOString(),chantier:j,client:cl||null,machine:m||grp.m||null,chauffeur:emp2?{id:emp2.id,name:emp2.name}:null,rapportWirtgen:mr||null};const blob=new Blob([JSON.stringify(exp,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='rapport_'+(machName||'chantier').replace(/\s/g,'_')+'_'+j.date+'.json';a.click();URL.revokeObjectURL(url)}} title="Exporter ce chantier + son rapport Wirtgen en JSON" style={{background:'none',border:'2px solid #0891b2',borderRadius:6,fontSize:13,cursor:'pointer',padding:'4px 12px',color:'#0891b2',fontWeight:700}}>📤 Export JSON</button>
<label style={{background:'none',border:'2px solid #ea580c',borderRadius:6,fontSize:13,cursor:'pointer',padding:'4px 12px',color:'#ea580c',fontWeight:700,position:'relative'}} title={'Ajouter des photos pour ce chantier'+(j.photos&&j.photos.length?' ('+j.photos.length+' deja)':'')}>📷 Photos{j.photos&&j.photos.length>0&&<span style={{marginLeft:6,background:'#ea580c',color:'#fff',borderRadius:10,padding:'0 7px',fontSize:11,fontWeight:800}}>{j.photos.length}</span>}<input type="file" accept="image/*" multiple style={{display:'none'}} onChange={async e=>{const files=Array.from(e.target.files||[]);if(!files.length)return;try{const dataUrls=await Promise.all(files.map(f=>compressImage(f,1280,0.75)));const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){if(!jj.photos)jj.photos=[];dataUrls.forEach(u=>{jj.photos.push({dataUrl:u,addedAt:new Date().toISOString()})});save(nd);alert('✓ '+dataUrls.length+' photo(s) ajoutee(s)')}}catch(err){alert('Erreur upload : '+err)}e.target.value=''}}/></label>
</div>
{(()=>{
...
<select value={j.startFrom||''} onChange={e=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.startFrom=e.target.value;const sel2=depOptions2.find(o=>o.id===e.target.value);jj.kmAller=sel2&&sel2.km?sel2.km:0;jj.travelMinAller=sel2&&sel2.km?Math.round((sel2.km/80)*60):0;jj.distanceKm=(jj.kmAller||0)+(jj.kmRetour||0);jj.travelMin=(jj.travelMinAller||0)+(jj.travelMinRetour||0);save(nd)}}} style={{fontSize:12,padding:'2px 5px',borderRadius:4,border:'1px solid #0891b240',background:j.startFrom&&shortDep&&j.startFrom===shortDep.id?'#0891b218':'#0891b208',color:'#0891b2',fontWeight:600,minWidth:110,maxWidth:160}}>
...
{!j.gps&&<button onClick={()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.gps=chantierGpsR.lat.toFixed(6)+','+chantierGpsR.lon.toFixed(6);save(nd)}}} style={{background:'#dcfce7',border:'1px solid #16a34a',borderRadius:4,padding:'1px 8px',cursor:'pointer',fontSize:10,color:'#14532d',fontWeight:700}}>↵ Enregistrer comme GPS chantier</button>}
    ...
    <div style={{padding:'12px 8px 10px',textAlign:'center',background:b.color+'08',flex:1,display:'flex',flexDirection:'column',justifyContent:'center'}}>
      ...
      {b.d.cR>0&&<div style={{fontSize:12,color:C.dim,marginTop:4,fontWeight:600,display:'flex',alignItems:'center',justifyContent:'center',gap:6,flexWrap:'wrap'}}>
        ...
        {bi<2&&(()=>{const isTransfert=bi===0;const fuelLbl=isTransfert?'Gazole':(machineFuelType==='gnr'?'GNR':'Gazole');const curPrice=isTransfert?prixGazole:prixMachineFuel;return<span style={{display:'inline-flex',alignItems:'center',gap:2}} title={'Prix '+fuelLbl+' (modifiable)'}>
          <input type="number" step="0.01" defaultValue={Number(curPrice).toFixed(3)} key={'fp_'+bi+'_'+curPrice} onBlur={e=>{const v=Number(e.target.value);if(!(v>0))return;if(Math.abs(v-curPrice)<0.001)return;const nd=JSON.parse(JSON.stringify(_liveData||data));const depId=isTransfert?(j.startFrom&&j.startFrom!=='home'?j.startFrom:null):j.machineFuelDepot;if(depId){const dep=(nd.depots||[]).find(d=>d.id===depId);if(dep){if(isTransfert||machineFuelType==='gazole')dep.gazolePrice=v;else dep.gnrPrice=v;save(nd);return}}nd.fuelPrice=v;save(nd)}} style={{width:54,fontSize:11,padding:'1px 3px',borderRadius:3,border:'1px solid #cbd5e1',textAlign:'right',background:'#fff',fontWeight:700}}/>
...
<button onClick={()=>{if(confirm('Supprimer cette photo ?')){const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj&&jj.photos){jj.photos.splice(pi,1);save(nd)}}}} style={{position:'absolute',top:4,right:4,background:'#fff',border:'1px solid #ef4444',color:'#dc2626',borderRadius:6,padding:'2px 6px',cursor:'pointer',fontSize:10,fontWeight:700,lineHeight:1}}>🗑</button>
</div>))}
</div>
</div>}
...
// ===================== VUE TABLEAU (10/09/2026) =====================
// Le planning a l'image de la feuille de papa : une carte par MACHINE, le ou les chauffeurs dans le carre de gauche,
// deux lignes de chantier minimum, ses cases (prevenu / nuit / paye / bon) et ses couleurs
// (prenom vert = prevenu, ligne verte = lu, client rose = paye, lieu rouge = nuit, forfait jaune = bon envoye).
// L'heure se tape dans le lieu ("... a 21h") et devient l'heure du chantier. Pas d'heure = pas d'heure.
const renderColTableau=(types,label)=>{
const getEmp=id=>(data.employees||[]).find(e=>e.id===id);
const wOf=m=>Number(getMachineWidth(m))||0;
// ordre : balayeuses avant citernes, raboteuses par largeur decroissante (210, 200, 150, 130, 100, 35) comme dans la feuille de papa
const ordreTab=data.machineOrderTableau||[];const rang=m=>{const i=ordreTab.indexOf(m.id);return i<0?9999:i};
const machs=(data.machines||[]).filter(m=>types.includes(m.type)).sort((a,b)=>(types.indexOf(a.type)-types.indexOf(b.type))||(rang(a)-rang(b))||(a.type==='Raboteuse'?(wOf(b)-wOf(a)):0)||String(a.name||'').localeCompare(String(b.name||'')));
// Glisser-deposer (mode choisi dans la barre du haut) : un chauffeur, un chantier ou une machine lache sur une carte
const deposerSur=(m)=>{if(!dragTab||!m)return;const nd=JSON.parse(JSON.stringify(_liveData||data));const drivers0=[...new Set(dayMissions.filter(j=>j.machineId===m.id).map(j=>j.employeeId).filter(Boolean))];const defEmp0=(nd.employees||[]).find(e=>e.machineId===m.id);const chauffeurCible=drivers0[0]||(defEmp0?defEmp0.id:'');
 if(dragTab.kind==='chantier'){const jj=(nd.jobs||[]).find(x=>x.id===dragTab.id);if(jj){jj.machineId=m.id;if(chauffeurCible)jj.employeeId=chauffeurCible;jj._updatedAt=Date.now()}}
 else if(dragTab.kind==='chauffeur'){const js=(nd.jobs||[]).filter(x=>x.date===selDate&&x.employeeId===dragTab.id&&x.machineId===dragTab.from&&x.type!=='depot');if(js.length)js.forEach(jj=>{jj.machineId=m.id;jj._updatedAt=Date.now()});else nd.jobs.push(gabarit(m,dragTab.id))}
 else if(dragTab.kind==='machine'){if(dragTab.id===m.id){setDragTab(null);return}const ids=machs.map(x=>x.id);const from=ids.indexOf(dragTab.id),to=ids.indexOf(m.id);if(from<0||to<0){setDragTab(null);return}ids.splice(from,1);ids.splice(to,0,dragTab.id);const autres=(nd.machineOrderTableau||[]).filter(id=>!ids.includes(id));nd.machineOrderTableau=[...autres,...ids]}
 save(nd);setDragTab(null)};
const dropProps=(m)=>({onDragOver:e=>{if(dragTab){e.preventDefault();e.stopPropagation()}},onDrop:e=>{if(dragTab){e.preventDefault();e.stopPropagation();deposerSur(m)}}});
const colJobs=dayMissions.filter(j=>{const m=getMach(j.machineId);return m&&types.includes(m.type)});
const caCol=colJobs.reduce((s,j)=>s+(j.priceForfait||0)+(j.hasTransfer?j.transferPrice||0:0),0);
const H=30;
const boxS=(w,extra)=>({fontSize:TF.size,fontWeight:TF.bold?700:400,fontFamily:'inherit',padding:'2px 7px',borderRadius:5,border:'1px solid '+C.border,background:'#fff',height:H,boxSizing:'border-box',display:'flex',alignItems:'center',gap:5,width:w||undefined,flex:w?'none':1,minWidth:w?undefined:140,color:C.text,whiteSpace:'nowrap',overflow:'hidden',outline:'none',...(extra||{})});
const Coche=({on,color,label,onClick,title})=>(<span title={title||label} onClick={onClick} style={{display:'flex',flexDirection:'column',alignItems:'center',width:28,flexShrink:0,lineHeight:1,cursor:onClick?'pointer':'default'}}><span style={{fontSize:8,fontWeight:700,color:C.dim,textTransform:'uppercase'}}>{label}</span><span style={{display:'inline-flex',width:17,height:17,borderRadius:4,background:on?color:'#fff',border:'2px solid '+(on?color:C.muted),boxSizing:'border-box',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:12,fontWeight:800}}>{on?'✓':''}</span></span>);
const heureDe=txt=>{const hm=/(\d{1,2})\s*h\s*(\d{2})?/i.exec(String(txt||''));if(!hm)return'';const h=Math.min(23,parseInt(hm[1],10));return pad2(h)+':'+(hm[2]||'00')};
const modif=(id,fn)=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=(nd.jobs||[]).find(x=>x.id===id);if(!jj)return;fn(jj,nd);save(nd)};
const gabarit=(m,empId)=>({id:uid(),date:selDate,employeeId:empId||'',machineId:m?m.id:'',clientId:'',agencyName:'',siteManager:'',siteManagerPhone:'',location:'',gps:'',forfaitType:'',priceForfait:0,isNight:false,hasTransfer:false,transferPrice:0,billingStart:'',startFrom:'',endAt:'',machineFuelL:0,machineFuelDepot:'',kmAller:0,kmRetour:0,travelMinAller:0,travelMinRetour:0,distanceKm:0,travelMin:0,sent:false});
const nouveauJob=(m,empId,field,value)=>{const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.jobs)nd.jobs=[];const nj=gabarit(m,empId);if(field)nj[field]=value;if(field==='location'){const h=heureDe(value);if(h)nj.billingStart=h}nd.jobs.push(nj);save(nd);return nj.id};
const petitBtn=(txt,title,onClick,color,actif)=>(<button title={title} onClick={onClick} style={{padding:'2px 5px',borderRadius:5,fontSize:11,border:'2px solid '+(actif?color:C.muted),background:actif?color+'20':'transparent',color:actif?color:C.dim,cursor:'pointer',fontWeight:700,lineHeight:1.2,flexShrink:0}}>{txt}</button>);
const ligneJob=(j,m,tag)=>{const cl=(data.clients||[]).find(c=>c.id===j.clientId);const nuit=!!j.isNight;const bg=nuit?'#fee2e2':(j.ack?'#dcfce7':'transparent');const rose=j.paid?{background:'#ffd6ff',borderColor:'#e879f9'}:{};const jaune=(j.bonEnvoye&&j.forfaitType)?{background:'#fef08a',borderColor:'#eab308',fontWeight:700}:{};const rouge=(nuit&&j.location)?{background:'#fecaca',borderColor:'#dc2626',color:'#7f1d1d',fontWeight:700}:{};const h=j.billingStart||heureDe(j.location);const gps=j.gps||j._geocodedGps;const _sg=j.signature;const fin=(_sg&&_sg.signedAt)?((()=>{const d=new Date(_sg.signedAt);const hh=pad2(d.getHours())+':'+pad2(d.getMinutes());const dm=_sg.durationMin;return hh+(dm!=null?' · '+Math.floor(dm/60)+'h'+pad2(dm%60):'')})()):'';
const drj=modeTab==='chantier';
const elNuit=(<Coche on={nuit} color={C.red} label="nuit" title="Chantier de nuit" onClick={()=>modif(j.id,(jj,nd)=>{jj.isNight=!jj.isNight;if(jj.priceForfait&&jj.forfaitType&&m){const p=getForfaitPrice(nd,jj.clientId,m,jj.forfaitType,jj.citOption,jj.isNight);if(p)jj.priceForfait=p}})}/>);
const elClient=(<input key={'tcl_'+j.id+'_'+(j.clientId||'_')} list="planning-clients-list" defaultValue={cl?cl.name:''} onChange={e=>{const v=e.target.value;const matched=(data.clients||[]).find(c2=>c2.name===v);if(!matched)return;modif(j.id,(jj,nd)=>{jj.clientId=matched.id;const m3=getMach(jj.machineId);if(m3&&jj.forfaitType){const p=getForfaitPrice(nd,matched.id,m3,jj.forfaitType,jj.citOption,jj.isNight);if(p)jj.priceForfait=p}})}} onBlur={e=>{const v=e.target.value.trim();const exists=(data.clients||[]).find(c2=>c2.name.toLowerCase()===v.toLowerCase());if(exists){if(j.clientId!==exists.id)modif(j.id,(jj,nd)=>{jj.clientId=exists.id;const m3=getMach(jj.machineId);if(m3&&jj.forfaitType){const p=getForfaitPrice(nd,exists.id,m3,jj.forfaitType,jj.citOption,jj.isNight);if(p)jj.priceForfait=p}});return}if(!v){if(j.clientId)modif(j.id,jj=>{jj.clientId=''});return}const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.clients)nd.clients=[];const nc={id:uid(),name:v,forfaitType:'standard',agencies:[],siteManagers:[]};nd.clients.push(nc);const jj=nd.jobs.find(x=>x.id===j.id);if(jj)jj.clientId=nc.id;save(nd)}} style={mob?{...boxS(0,rose),flex:1,minWidth:0}:boxS(118,rose)}/>);
const elChef=(<select value={j.siteManager||''} onChange={e=>{if(e.target.value==='__new__'){const n=prompt('Nouveau chef chantier'+(j.agencyName?' (agence : '+j.agencyName+')':'')+' :');if(n){modif(j.id,(jj,nd)=>{jj.siteManager=n;const cl2=(nd.clients||[]).find(c2=>c2.id===j.clientId);if(cl2){if(!cl2.siteManagers)cl2.siteManagers=[];if(!cl2.siteManagers.find(sm=>sm.name===n&&(sm.agency||'')===(j.agencyName||''))){const ph=prompt('Tel du chef (optionnel):','')||'';cl2.siteManagers.push({name:n,phone:ph,agency:j.agencyName||''});jj.siteManagerPhone=ph}}})}}else{const val=e.target.value;modif(j.id,jj=>{jj.siteManager=val;const cl2=(data.clients||[]).find(c2=>c2.id===j.clientId);const sm=(cl2&&cl2.siteManagers||[]).find(x=>x.name===val&&(!j.agencyName||!x.agency||x.agency===j.agencyName));if(sm)jj.siteManagerPhone=sm.phone||''})}}} title={j.siteManagerPhone?'Tel : '+j.siteManagerPhone:'Chef de chantier'} style={boxS(mob?96:108,{...rose,padding:'2px 4px',color:j.siteManagerPhone?'#0f766e':C.text})}>
<option value=""></option>{(cl&&cl.siteManagers||[]).filter(x=>!j.agencyName||!x.agency||x.agency===j.agencyName).map((x,si)=><option key={si} value={x.name}>{x.name}{x.phone?' ☎':''}</option>)}<option value="__new__">+ Nouveau...</option>
</select>);
const elLieu=(<div style={{position:'relative',flex:1,minWidth:140,display:'flex'}}><input value={j.location||''} onChange={e=>{const v=e.target.value;const tgt=e.target;modif(j.id,jj=>{jj.location=v;if(jj._geocodedGps)jj._geocodedGps='';const hh=heureDe(v);if(hh)jj.billingStart=hh});triggerAutoGeocode(j.id,v,tgt)}} onFocus={e=>{if(j.location&&j.location.trim().length>=3&&!j._geocodedGps)triggerAutoGeocode(j.id,j.location,e.target)}} title={gps?'GPS : '+gps:'Lieu (tape l heure dedans : ... a 21h)'} style={{...boxS(0,rouge),width:'100%',minWidth:0,paddingLeft:gps?24:7,paddingRight:fin?(mob?118:160):62}}/>{gps&&<span style={{position:'absolute',left:6,top:6,fontSize:13,lineHeight:1,pointerEvents:'none'}}>📍</span>}{/* l'heure : lue dans le lieu, ou saisie ici directement ; a sa droite, la fin de chantier pointee et le temps passe (23/09/2026) */}<div style={{position:'absolute',right:4,top:4,height:H-8,display:'flex',alignItems:'center',gap:4}}><input type="time" className="rm-heure" value={h||''} onChange={e=>{const v=e.target.value||'';modif(j.id,jj=>{jj.billingStart=v})}} title="Heure de début du chantier" style={{height:H-8,fontSize:11,fontWeight:700,color:'#0f766e',background:h?'#ccfbf1':'#f1f5f9',border:'none',borderRadius:4,padding:'0 4px',lineHeight:1.2,width:52,boxSizing:'border-box',fontFamily:'inherit',cursor:'pointer'}}/>{fin&&<span title="Fin de chantier pointée par le chauffeur · temps passé" style={{height:H-8,display:'inline-flex',alignItems:'center',fontSize:11,fontWeight:700,color:'#15803d',background:'#dcfce7',borderRadius:4,padding:'0 5px',whiteSpace:'nowrap'}}>{fin}</span>}</div></div>);
const elLieuMob=(<div style={{position:'relative',flex:1,minWidth:0,display:'flex'}}><input value={j.location||''} onChange={e=>{const v=e.target.value;const tgt=e.target;modif(j.id,jj=>{jj.location=v;if(jj._geocodedGps)jj._geocodedGps='';const hh=heureDe(v);if(hh)jj.billingStart=hh});triggerAutoGeocode(j.id,v,tgt)}} onFocus={e=>{if(j.location&&j.location.trim().length>=3&&!j._geocodedGps)triggerAutoGeocode(j.id,j.location,e.target)}} title={gps?'GPS : '+gps:'Lieu (tape l heure dedans : ... a 21h)'} style={{...boxS(0,rouge),width:'100%',minWidth:0,paddingLeft:gps?24:7,paddingRight:7}}/>{gps&&<span style={{position:'absolute',left:6,top:6,fontSize:13,lineHeight:1,pointerEvents:'none'}}>📍</span>}</div>);
const elHeureMob=(<div style={{height:H-8,display:'flex',alignItems:'center',gap:4}}><input type="time" className="rm-heure" value={h||''} onChange={e=>{const v=e.target.value||'';modif(j.id,jj=>{jj.billingStart=v})}} title="Heure de début du chantier" style={{height:H-8,fontSize:11,fontWeight:700,color:'#0f766e',background:h?'#ccfbf1':'#f1f5f9',border:'none',borderRadius:4,padding:'0 4px',lineHeight:1.2,width:52,boxSizing:'border-box',fontFamily:'inherit',cursor:'pointer'}}/>{fin&&<span title="Fin de chantier pointée par le chauffeur · temps passé" style={{height:H-8,display:'inline-flex',alignItems:'center',fontSize:11,fontWeight:700,color:'#15803d',background:'#dcfce7',borderRadius:4,padding:'0 5px',whiteSpace:'nowrap'}}>{fin}</span>}</div>);
const elForfait=(<select value={j.forfaitType||''} title="Forfait" onChange={e=>{const val=e.target.value;modif(j.id,(jj,nd)=>{jj.forfaitType=val;if(!m)return;const p=getForfaitPrice(nd,jj.clientId,m,val,jj.citOption,jj.isNight);if(p)jj.priceForfait=p;else if(val){let k=getForfaitKey(nd,jj.clientId,m)||'?';if(m.type==='Citerne')k+='_'+(jj.citOption||'Avec chauffeur');const cn=((nd.clients||[]).find(c=>c.id===jj.clientId)||{}).name||'(pas de client)';setTimeout(()=>alert('⚠ Aucun tarif configure\n\nMachine : '+m.name+'\nForfait : '+val+'\nClient : '+cn+'\n\nCle recherchee : '+k+'\n\nVa dans Forfaits pour saisir le tarif.'),50)}})}} style={boxS(50,{...jaune,padding:'2px 2px',textAlign:'center'})}><option value=""></option>{(m&&m.type==='Citerne'?['Demi-journee','Journee']:['2h','4h','6h','8h','10h']).map(f=><option key={f} value={f}>{f}</option>)}</select>);
const elT=petitBtn('T','Transfert',()=>modif(j.id,(jj,nd)=>{jj.hasTransfer=!jj.hasTransfer;if(jj.hasTransfer&&!jj.transferPrice&&m){const tp=getTransferPrice(nd,jj.clientId,m,jj.citOption,jj.isNight);jj.transferPrice=tp||0}}),C.purple,!!j.hasTransfer);
const elBon=(<Coche on={!!j.bonEnvoye} color="#ca8a04" label="bon" title="Bon envoyé" onClick={()=>modif(j.id,jj=>{jj.bonEnvoye=!jj.bonEnvoye})}/>);
const elX=(<button title="Supprimer" onClick={()=>{if(confirm('Supprimer ?')){const nd=JSON.parse(JSON.stringify(_liveData||data));tombstone(nd,'jobs',j.id);nd.jobs=nd.jobs.filter(x=>x.id!==j.id);save(nd)}}} style={{background:'none',border:'none',cursor:'pointer',fontSize:16,color:C.red,padding:'0 2px',lineHeight:1}}>×</button>);
if(mob)return(<div key={j.id} style={{padding:'4px 6px',borderBottom:'1px solid '+C.border+'66',background:bg}}>{tag?<div style={{fontSize:11,fontWeight:700,color:C.dim,marginBottom:2}}>{tag}</div>:null}<div style={{display:'flex',alignItems:'center',gap:5,marginBottom:4}}>{elNuit}{elClient}{elChef}</div><div style={{display:'flex',alignItems:'center',gap:5,marginBottom:4}}>{elLieuMob}</div><div style={{display:'flex',alignItems:'center',gap:5}}>{elHeureMob}<span style={{flex:1}}></span>{elForfait}{elT}{elBon}{elX}</div></div>);
return(<div key={j.id} draggable={drj} onDragStart={e=>{if(!drj)return;e.stopPropagation();setDragTab({kind:'chantier',id:j.id});e.dataTransfer.effectAllowed='move'}} style={{display:'flex',alignItems:'center',gap:5,padding:'3px 6px',borderBottom:'1px solid '+C.border+'66',background:bg,height:H+8,boxSizing:'border-box',cursor:drj?'grab':'default',outline:drj?'2px dashed #f59e0b':'none',outlineOffset:-2}}>{elNuit}{elClient}{elChef}{elLieu}{elForfait}{elT}{elBon}{elX}</div>)};
const ligneDepot=(dj)=>{const dep=getDepot(dj.depotId);const isRest=dj.rest||dj.depotActivity==='Repos';return(<div key={dj.id} style={{display:'flex',alignItems:'center',gap:8,padding:'3px 10px',height:H+8,boxSizing:'border-box',background:isRest?'#fee2e2':'#f1f5f9',borderBottom:'1px solid '+C.border+'66'}}><span style={{fontSize:14,fontWeight:700,color:isRest?C.red:'#64748b'}}>{isRest?'😴 REPOS':'🏭 Dépôt'+(dep?' — '+dep.name:'')+(dj.depotActivity?' — '+dj.depotActivity:'')+(dj.depotDescription?' ('+dj.depotDescription+')':'')}</span><button onClick={()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));tombstone(nd,'jobs',dj.id);nd.jobs=nd.jobs.filter(x=>x.id!==dj.id);save(nd)}} style={{marginLeft:'auto',background:'none',border:'none',cursor:'pointer',fontSize:16,color:C.red}}>×</button></div>)};
const ligneVide=(m,empId,k)=>{
const vNuit=(<Coche on={false} color={C.red} label="nuit" onClick={()=>nouveauJob(m,empId,'isNight',true)}/>);
const vClient=(<input list="planning-clients-list" onBlur={e=>{const v=e.target.value.trim();if(!v)return;const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.clients)nd.clients=[];let c=nd.clients.find(c2=>c2.name.toLowerCase()===v.toLowerCase());if(!c){c={id:uid(),name:v,forfaitType:'standard',agencies:[],siteManagers:[]};nd.clients.push(c)}if(!nd.jobs)nd.jobs=[];nd.jobs.push({...gabarit(m,empId),clientId:c.id});save(nd);e.target.value=''}} style={mob?{...boxS(0),flex:1,minWidth:0}:boxS(118)}/>);
const vChef=(<select disabled style={boxS(mob?96:108,{padding:'2px 4px',background:'#f8fafc'})}><option value=""></option></select>);
const vLieu=(<input onBlur={e=>{const v=e.target.value.trim();if(v){nouveauJob(m,empId,'location',v);e.target.value=''}}} onKeyDown={e=>{if(e.key==='Enter'&&e.target.value.trim()){nouveauJob(m,empId,'location',e.target.value.trim());e.target.value=''}}} style={boxS(0)}/>);
const vForfait=(<select value="" onChange={e=>{if(e.target.value)nouveauJob(m,empId,'forfaitType',e.target.value)}} style={boxS(50,{padding:'2px 2px',textAlign:'center'})}><option value=""></option>{(m&&m.type==='Citerne'?['Demi-journee','Journee']:['2h','4h','6h','8h','10h']).map(f=><option key={f} value={f}>{f}</option>)}</select>);
const vBon=(<Coche on={false} color="#ca8a04" label="bon"/>);
if(mob)return(<div key={'v'+k} style={{padding:'4px 6px',borderBottom:'1px solid '+C.border+'66'}}><div style={{display:'flex',alignItems:'center',gap:5,marginBottom:4}}>{vNuit}{vClient}{vChef}</div><div style={{display:'flex',alignItems:'center',gap:5,marginBottom:4}}>{vLieu}</div><div style={{display:'flex',alignItems:'center',gap:5,justifyContent:'flex-end'}}>{vForfait}{vBon}</div></div>);
return(<div key={'v'+k} style={{display:'flex',alignItems:'center',gap:5,padding:'3px 6px',borderBottom:'1px solid '+C.border+'66',height:H+8,boxSizing:'border-box'}}>{vNuit}{vClient}{vChef}{vLieu}{vForfait}{vBon}</div>)};
const carte=(m,jobsForce)=>{const col=m?widthColor(m):C.muted;const mid=m?m.id:'';const jobs=(jobsForce||dayMissions.filter(j=>j.machineId===mid)).slice().sort((a,b)=>String(a.billingStart||'99').localeCompare(String(b.billingStart||'99')));
const drivers=[...new Set(jobs.map(j=>j.employeeId).filter(Boolean))];const defEmp=m?(data.employees||[]).find(e=>e.machineId===m.id):null;if(!drivers.length&&defEmp)drivers.push(defEmp.id);
const depotRows=m?dayJobs.filter(j=>j.type==='depot'&&drivers.includes(j.employeeId)&&((getEmp(j.employeeId)||{}).machineId===m.id)):[];
const rows=[...jobs.map(j=>({job:j})),...depotRows.map(j=>({dep:j}))];while(rows.length<2)rows.push({vide:true});
const multi=drivers.length>1;const pickKey='t_'+(mid||'sans');
const picker=(onPick)=>addEmpOpen===pickKey?(<div onClick={e=>e.stopPropagation()} style={{position:'absolute',top:'100%',left:0,zIndex:999,background:'#fff',border:'1px solid '+C.border,borderRadius:8,boxShadow:'0 4px 20px #0003',minWidth:150,maxHeight:280,overflowY:'auto'}}>{(data.employees||[]).filter(e2=>e2.role!=='mechanic').map(e2=><div key={e2.id} onClick={()=>{onPick(e2.id);setAddEmpOpen(null)}} style={{padding:'6px 12px',cursor:'pointer',fontSize:14,borderBottom:'1px solid '+C.border}}>{e2.name}</div>)}</div>):null;
const btnPrev=(js)=>{const all=js.length>0&&js.every(j=>j.sent);const some=js.some(j=>j.sent);return(<span title={js.length?(all?'Chantier envoyé au chauffeur — recliquer pour renvoyer':'Prévenir le chauffeur : envoi du chantier sur son téléphone'):'Aucun chantier à envoyer'} onClick={async()=>{if(!js.length)return;for(const j of js){await envoyerChantier(j)}}} style={{display:'inline-flex',width:17,height:17,borderRadius:4,background:all?'#16a34a':'#fff',border:'2px solid '+(all?'#16a34a':(some?'#d97706':C.muted)),boxSizing:'border-box',alignItems:'center',justifyContent:'center',flexShrink:0,cursor:js.length?'pointer':'default',color:'#fff',fontSize:12,fontWeight:800}}>{all?'✓':''}</span>)};
const bloc=(empId,js)=>{const emp=getEmp(empId);const prev=js.length>0&&js.every(j=>j.sent);const dr=modeTab==='chauffeur'&&!!emp;return(<div draggable={dr} onDragStart={e=>{if(!dr)return;e.stopPropagation();setDragTab({kind:'chauffeur',id:empId,from:mid});e.dataTransfer.effectAllowed='move'}} style={{flex:1,display:'flex',alignItems:'center',gap:6,padding:'0 6px',background:prev?'#00FF00':'transparent',minWidth:0,cursor:dr?'grab':'default',outline:dr?'2px dashed #f59e0b':'none',outlineOffset:-2}}>{emp?<React.Fragment>{btnPrev(js)}<div title={emp.name} style={{fontSize:TF.size+2,fontWeight:800,color:C.text,lineHeight:1.1,whiteSpace:'nowrap',overflow:'hidden'}}>{emp.name}</div></React.Fragment>:<div onClick={e=>{e.stopPropagation();setAddEmpOpen(addEmpOpen===pickKey?null:pickKey)}} title="Choisir le chauffeur" style={{width:96,height:24,border:'1px solid '+C.border,borderRadius:5,background:'#fff',cursor:'pointer'}}></div>}</div>)};
const machLabel=<div style={{display:'flex',alignItems:'center',padding:'0 6px 0 2px'}}><div style={{fontSize:12,fontWeight:800,color:col,whiteSpace:'nowrap'}}>{m?m.name:'sans machine'}</div></div>;
let gauche;
if(!multi){const empId=drivers[0]||'';gauche=(<div style={{width:168,minWidth:168,display:'flex',borderRight:'2px solid '+col+'30',background:col+'10',position:'relative'}}>{bloc(empId,jobs)}{machLabel}
{empId?<span title="Ajouter un chauffeur sur l'autre ligne" onClick={e=>{e.stopPropagation();setAddEmpOpen(addEmpOpen===pickKey?null:pickKey)}} style={{position:'absolute',right:4,bottom:2,width:14,height:14,borderRadius:4,border:'1.5px dashed '+C.muted,color:C.muted,fontSize:11,fontWeight:700,display:'inline-flex',alignItems:'center',justifyContent:'center',lineHeight:1,cursor:'pointer'}}>+</span>:null}
{picker(eid=>{const sansChauffeur=jobs.filter(j=>!j.employeeId);if(!drivers.length&&sansChauffeur.length){modif(sansChauffeur[0].id,jj=>{jj.employeeId=eid})}else{nouveauJob(m,eid)}})}
</div>)}
else{gauche=(<div style={{width:168,minWidth:168,display:'flex',borderRight:'2px solid '+col+'30',background:col+'10',position:'relative'}}><div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0}}>{rows.map((r,i)=>{const empId=r.job?r.job.employeeId:(r.dep?r.dep.employeeId:'');const js=r.job?[r.job]:[];return(<React.Fragment key={i}>{i>0&&<div style={{height:2,background:col+'55'}}></div>}{bloc(empId,js)}</React.Fragment>)})}</div>{machLabel}{picker(eid=>nouveauJob(m,eid))}</div>)}
// Colonne « payé » tout a gauche : un bouton par ligne de chantier, aligne sur sa ligne (23/09/2026)
const payeBtn=(job)=>(<span title={job.paid?'Client a payé':'Marquer client payé'} onClick={()=>modif(job.id,jj=>{jj.paid=!jj.paid})} style={{display:'inline-flex',width:18,height:18,borderRadius:4,background:job.paid?C.purple:'#fff',border:'2px solid '+(job.paid?C.purple:C.muted),boxSizing:'border-box',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:12,fontWeight:800,cursor:'pointer'}}>{job.paid?'€':''}</span>);
const colPaye=(<div style={{width:26,minWidth:26,display:'flex',flexDirection:'column',borderRight:'1px solid '+col+'30',background:col+'08'}}>{rows.map((r,i)=>(<div key={'p'+i} style={{height:H+8,display:'flex',alignItems:'center',justifyContent:'center'}}>{r.job?payeBtn(r.job):null}</div>))}</div>);
if(mob){const rowsMob=[...jobs.map(j=>({job:j})),...depotRows.map(j=>({dep:j}))];if(drivers.length||rowsMob.length)rowsMob.push({vide:true});const entete=(<div style={{display:'flex',alignItems:'center',background:col+'10',borderBottom:'2px solid '+col+'30',padding:'3px 6px',gap:4,position:'relative'}}>{drivers.length?drivers.map((eid,i)=><React.Fragment key={eid}>{i>0&&<span style={{color:col,fontWeight:800}}>·</span>}{bloc(eid,jobs.filter(j=>j.employeeId===eid))}</React.Fragment>):bloc('',jobs)}{machLabel}<span title="Ajouter un chauffeur" onClick={e=>{e.stopPropagation();setAddEmpOpen(addEmpOpen===pickKey?null:pickKey)}} style={{width:18,height:18,borderRadius:4,border:'1.5px dashed '+C.muted,color:C.muted,fontSize:12,fontWeight:700,display:'inline-flex',alignItems:'center',justifyContent:'center',cursor:'pointer',marginLeft:4,flexShrink:0}}>+</span>{picker(eid=>{const sansChauffeur=jobs.filter(j=>!j.employeeId);if(!drivers.length&&sansChauffeur.length){modif(sansChauffeur[0].id,jj=>{jj.employeeId=eid})}else{nouveauJob(m,eid)}})}</div>);
return(<div key={mid||'sans'} style={{background:'#fff',borderRadius:8,border:'2px solid '+col,borderLeft:'6px solid '+col,marginBottom:6,position:'relative'}}>{entete}{rowsMob.map((r,i)=><div key={i} style={{display:'flex'}}><div style={{width:26,minWidth:26,display:'flex',alignItems:'center',justifyContent:'center',borderRight:'1px solid '+col+'30',background:col+'08'}}>{r.job?payeBtn(r.job):null}</div><div style={{flex:1,minWidth:0}}>{r.job?ligneJob(r.job,m,multi?((getEmp(r.job.employeeId)||{}).name||''):''):(r.dep?ligneDepot(r.dep):ligneVide(m,drivers[0]||'',i))}</div></div>)}</div>)}
const drm=modeTab==='machine'&&!!m;return(<div key={mid||'sans'} draggable={drm} onDragStart={e=>{if(!drm)return;setDragTab({kind:'machine',id:m.id});e.dataTransfer.effectAllowed='move'}} {...(m?dropProps(m):{})} style={{background:'#fff',borderRadius:8,border:'2px solid '+col,borderLeft:'6px solid '+col,boxShadow:'0 2px 6px rgba(0,0,0,.06)',display:'flex',marginBottom:5,position:'relative',cursor:drm?'grab':'default',outline:drm?'2px dashed #f59e0b':'none',outlineOffset:2}}>{colPaye}{gauche}<div style={{flex:1,minWidth:0}}>{rows.map((r,i)=>r.job?ligneJob(r.job,m):(r.dep?ligneDepot(r.dep):ligneVide(m,drivers[0]||'',i)))}</div></div>)};
const sansMachine=types.includes('Raboteuse')?dayMissions.filter(j=>!getMach(j.machineId)):[];
return(<div>
{machs.map((m,i)=><React.Fragment key={m.id}>{i>0&&machs[i-1].type!==m.type&&<div style={{display:'flex',alignItems:'center',gap:8,margin:'8px 0 6px'}}><span style={{color:'#fff',fontWeight:800,fontSize:13,letterSpacing:.4,textTransform:'uppercase'}}>{m.type==='Citerne'?'Citernes':m.type+'s'}</span><span style={{flex:1,height:1,background:'#ffffff33'}}></span></div>}{carte(m)}</React.Fragment>)}
{sansMachine.length>0&&carte(null,sansMachine)}
</div>)};
return(
<div>
{/* Barre du haut compacte : [bouton pannes] [< Date >] ... [CA] [📥 JD] */}
{PL.vue==='classique'&&<div style={{display:'flex',alignItems:'center',marginBottom:12,flexWrap:'wrap',gap:8}}>
...
{[['Raboteuses',caType('Raboteuse'),MC.Raboteuse],['Balayeuses',caType('Balayeuse'),MC.Balayeuse],['Citernes',caType('Citerne'),MC.Citerne],['Total',caTotal,C.text]].map(([lb,v,cc])=><div key={lb} style={{background:C.card,borderRadius:8,padding:'4px 10px',border:'1px solid '+C.border,display:'flex',flexDirection:'column',alignItems:'center',minWidth:86}}><span style={{fontSize:10,color:C.dim,textTransform:'uppercase',letterSpacing:.3}}>{lb}</span><span style={{fontWeight:800,color:cc,fontSize:15,lineHeight:1.1}}>{fmtMoney(v)}</span></div>)}
</div>
<button onClick={()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));nd.planningLayout={...(nd.planningLayout||{}),vue:PL.vue==='classique'?'tableau':'classique'};save(nd)}} style={{...btnStyle(C.dim),fontSize:12,padding:'6px 10px'}} title="Basculer entre la vue tableau (comme la feuille de papa) et l ancienne vue">{PL.vue==='classique'?'Vue tableau':'Vue classique'}</button>
...
<span style={{fontWeight:800,fontSize:mob?17:22,color:'#fff',padding:'2px 4px',letterSpacing:'0.3px',whiteSpace:'nowrap'}}>{fmtDate(new Date(selDate))}</span>
<button onClick={()=>navDate(1)} style={{...btnStyle(C.dim),padding:'6px 12px'}}>{'>'}</button>
{!mob&&<input type="date" value={selDate} onChange={e=>setSelDate(e.target.value)} style={{...inputStyle,width:122,marginLeft:2,padding:'5px 6px'}}/>}
</div>
{/* Milieu : chiffre d'affaires, forfaits et transferts separes, fond sombre, reduit pour tenir entre la date et les boutons */}
{mob?<div style={{flexBasis:'100%',order:2,display:'flex',gap:10,flexWrap:'wrap',fontSize:12,color:'#fff',padding:'2px 4px'}}>{rows.map(([lb,v,cc])=><span key={lb} style={{whiteSpace:'nowrap'}}><b style={{color:cc}}>{lb.slice(0,3)}</b> {((v.f+v.t)||0).toLocaleString('fr-FR',{maximumFractionDigits:0})} €</span>)}<span style={{whiteSpace:'nowrap'}}><b style={{color:'#fbbf24'}}>Total</b> {((tot.f+tot.t)||0).toLocaleString('fr-FR',{maximumFractionDigits:0})} €</span></div>
...
{!mob&&modeBtn('chauffeur','👤 Chauffeur','Déplacer un chauffeur : glisse son nom sur une autre carte')}
{!mob&&modeBtn('chantier','🚧 Chantier','Déplacer un chantier : glisse sa ligne sur une autre carte')}
{!mob&&modeBtn('machine','🚜 Machine','Déplacer une machine : glisse sa carte à la place d une autre')}
<button onClick={()=>setShowFont(!showFont)} title="Taille, gras et style d écriture du planning" style={{...btnStyle(showFont?'#f59e0b':C.dim,showFont),fontSize:12,padding:'5px 9px',fontWeight:800}}>Aa</button>
{showFont&&<div onClick={e=>e.stopPropagation()} style={{position:'absolute',top:'110%',right:0,zIndex:1000,background:'#fff',border:'1px solid '+C.border,borderRadius:8,boxShadow:'0 6px 20px #0004',padding:10,display:'flex',flexDirection:'column',gap:8,minWidth:230}}>
<div style={{display:'flex',alignItems:'center',gap:6}}><span style={{fontSize:12,color:C.dim,width:60}}>Taille</span><button onClick={()=>setTabFont({size:Math.max(10,TF.size-1)})} style={{...btnStyle(C.dim),padding:'2px 10px'}}>A-</button><b style={{width:28,textAlign:'center'}}>{TF.size}</b><button onClick={()=>setTabFont({size:Math.min(22,TF.size+1)})} style={{...btnStyle(C.dim),padding:'2px 10px'}}>A+</button></div>
<label style={{display:'flex',alignItems:'center',gap:8,fontSize:13,cursor:'pointer'}}><input type="checkbox" checked={!!TF.bold} onChange={e=>setTabFont({bold:e.target.checked})}/> Gras</label>
...
{!mob&&<button onClick={()=>{if(document.fullscreenElement){document.exitFullscreen()}else{document.documentElement.requestFullscreen().catch(()=>{})}}} style={{...btnStyle('#7c3aed'),fontSize:12,padding:'5px 9px'}} title="Plein ecran (Echap pour sortir)">⛶</button>}
{!mob&&typeof setSbHidden==='function'&&<button onClick={()=>setSbHidden(!sbHidden)} style={{...btnStyle('#0f766e'),fontSize:12,padding:'5px 9px'}} title={sbHidden?'Reafficher le menu':'Masquer le menu'}>{sbHidden?'▶':'◀'}</button>}
</div>
</div>)})()}
...
:<GardeVue onReset={()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));nd.planningLayout={...(nd.planningLayout||{}),vue:'classique'};save(nd)}}>
<div ref={fitRef} onClick={()=>{if(showFont)setShowFont(false)}} style={{display:'grid',gridTemplateColumns:mob?'minmax(0,1fr)':'1fr 1fr',gap:mob?6:12,alignItems:'start',fontFamily:TF.family}}>
<style>{'.rm-heure::-webkit-calendar-picker-indicator{display:none;-webkit-appearance:none}.rm-heure::-webkit-datetime-edit{padding:0}'}</style>
<datalist id="planning-clients-list">{(data.clients||[]).map(c2=><option key={c2.id} value={c2.name}/>)}</datalist>
{renderColTableau(['Raboteuse'],'Raboteuses')}
{renderColTableau(['Balayeuse','Citerne'],'Balayeuses + Citernes')}
</div>
</GardeVue>}
...
{dupJobId&&(()=>{const srcJob=(data.jobs||[]).find(j=>j.id===dupJobId);if(!srcJob)return null;const empO=(data.employees||[]).find(e=>e.id===srcJob.employeeId);const empN=empO?empO.name:'?';const clO=getClient(srcJob.clientId);const clN=clO?clO.name:'?';const doDup=()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.jobs)nd.jobs=[];const baseDate=new Date(srcJob.date);for(let i=1;i<=dupDays;i++){const d=new Date(baseDate);d.setDate(d.getDate()+i);const ds=fmtDateISO(d);const exists=nd.jobs.some(j2=>j2.employeeId===srcJob.employeeId&&j2.machineId===srcJob.machineId&&j2.clientId===srcJob.clientId&&j2.date===ds);if(!exists){const nj={...JSON.parse(JSON.stringify(srcJob)),id:uid(),date:ds,sent:false,ack:false};nd.jobs.push(nj)}}save(nd);setDupJobId(null)};return(
...
const calcCosts=(jobList)=>{let total=0;
// Pour les chauffeurs mensualises : 1 seule fois par jour-chauffeur (sinon on compte le salaire mensuel autant de fois qu'il y a de chantiers)
const empDayCounted={};
jobList.forEach(j=>{const emp=(data.employees||[]).find(e=>e.id===j.employeeId);const m=(data.machines||[]).find(x=>x.id===j.machineId);const isMonthly=emp&&emp.salaryType==='monthly';const hourly=isMonthly?0:(emp?Number(emp.hourlySalary)||0:0);const chargesRate=emp?Number(emp.chargesRate)||45:45;const mealA=emp?Number(emp.mealAllowance)||12:12;let salCharges;if(isMonthly){const key=(emp.id||'')+'_'+j.date;if(empDayCounted[key]){salCharges=0}else{empDayCounted[key]=true;const dailySal=(Number(emp.monthlySalary)||0)/wdpm;salCharges=dailySal*(1+chargesRate/100)}}else{const fh=forfaitHours(j.forfaitType);const salBrut=fh*hourly;salCharges=salBrut*(1+chargesRate/100)}const fuelType=getMachineFuelType(data,j.machineId);const truck2=(data.trucks||[]).find(t=>emp&&t.id===emp.truckId);const truckC=truck2?Number(truck2.fuelPer100)||25:25;const trajKm=(j.distanceKm||0);const trajL=(trajKm/100)*truckC;const fuelPr=getFuelPrice(data,fuelType,j.startFrom!=='home'?j.startFrom:null);const trajCost=trajL*fuelPr;const machFuelPr=getFuelPrice(data,fuelType,j.machineFuelDepot);const machCost=(j.machineFuelL||0)*machFuelPr;const assM=m?((m.insuranceMonthly||0)/wdpm):0;const credM=m?(Number(m.creditMonthly)||0)/wdpm:0;const ctM=m?((m.ctCost||0)/12)/wdpm:0;const assT=truck2?((truck2.insuranceMonthly||0)/wdpm):0;const credT=truck2?(Number(truck2.creditMonthly)||0)/wdpm:0;const ctT=truck2?((truck2.ctCost||0)/12)/wdpm:0;total+=salCharges+mealA+trajCost+machCost+assM+credM+ctM+assT+credT+ctT});return total};
...
const driverStats=useMemo(()=>{const empIds=[...new Set(jobs.map(j=>j.employeeId))];return empIds.map(eId=>{const emp=(data.employees||[]).find(e=>e.id===eId);if(!emp)return null;const eJobs=jobs.filter(j=>j.employeeId===eId);const ca=eJobs.reduce((s,j)=>s+(j.priceForfait||0)+(j.hasTransfer?j.transferPrice||0:0),0);const isMonthly=emp.salaryType==='monthly';const chargesRate=Number(emp.chargesRate)||45;const workDays=[...new Set(eJobs.map(j=>j.date))].length;let salTotal;if(isMonthly){const dailySal=(Number(emp.monthlySalary)||0)/wdpm;salTotal=dailySal*workDays}else{const hourly=Number(emp.hourlySalary)||0;const te=(data.timeEntries||[]).filter(t=>t.empId===eId&&t.date>=range.start&&t.date<=range.end);let totalWorkMin=0;te.forEach(t=>{totalWorkMin+=calcWorkedMin(t)});salTotal=(totalWorkMin/60)*hourly}const salCharges=salTotal*(1+chargesRate/100);const ratio=salCharges>0?(ca/salCharges):0;return{name:emp.name,ca,salCharges,missions:eJobs.length,days:workDays,ratio,initial:(emp.name||'?')[0].toUpperCase()}}).filter(Boolean).sort((a,b)=>b.ca-a.ca)},[data,jobs,range,wdpm]);
...
const addItem=(type)=>{const nm=(newName[type]||'').trim();if(!nm)return;const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.equipmentLists)nd.equipmentLists={Raboteuse:[],Balayeuse:[],Citerne:[]};if(!nd.equipmentLists[type])nd.equipmentLists[type]=[];nd.equipmentLists[type].push({id:uid(),name:nm});save(nd);setNewName({...newName,[type]:''})};
const delItem=(type,id)=>{if(!confirm('Supprimer cet equipement ?'))return;const nd=JSON.parse(JSON.stringify(_liveData||data));nd.equipmentLists[type]=((nd.equipmentLists||{})[type]||[]).filter(x=>x.id!==id);save(nd)};
const renameItem=(type,id,name)=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const arr=((nd.equipmentLists||{})[type]||[]);const idx=arr.findIndex(x=>x.id===id);if(idx>=0){arr[idx]={...arr[idx],name};save(nd)}};
...
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}><h4 style={{margin:0}}>Chefs chantier</h4><button style={btnStyle(C.accent)} onClick={()=>setSel({...sel,siteManagers:[...sel.siteManagers,{name:'',phone:'',email:'',agency:''}]})}>+</button></div>
{sel.siteManagers.map((s,i)=>(
<div key={i} style={{display:'flex',gap:6,alignItems:'flex-start',marginBottom:8,flexWrap:'wrap',padding:'8px',background:'#f8fafc',borderRadius:8,border:'1px solid '+C.border}}>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,flex:1,minWidth:180}}>
<input style={{...inputStyle}} placeholder="Nom" value={s.name} onChange={e=>{const ns=[...sel.siteManagers];ns[i]={...ns[i],name:e.target.value};setSel({...sel,siteManagers:ns})}}/>
<input style={{...inputStyle}} placeholder="Agence" value={s.agency||''} onChange={e=>{const ns=[...sel.siteManagers];ns[i]={...ns[i],agency:e.target.value};setSel({...sel,siteManagers:ns})}}/>
<input style={{...inputStyle}} type="tel" placeholder="📞 Telephone" value={s.phone||''} onChange={e=>{const ns=[...sel.siteManagers];ns[i]={...ns[i],phone:e.target.value};setSel({...sel,siteManagers:ns})}}/>
<input style={{...inputStyle}} type="email" placeholder="📧 Email (pour facture)" value={s.email||''} onChange={e=>{const ns=[...sel.siteManagers];ns[i]={...ns[i],email:e.target.value};setSel({...sel,siteManagers:ns})}}/>
</div>
<button style={{background:'none',border:'none',cursor:'pointer',color:C.red,fontSize:18,padding:'4px 8px',fontWeight:700}} onClick={()=>setSel({...sel,siteManagers:sel.siteManagers.filter((_,j)=>j!==i)})}>×</button>
</div>))}
</div>
<div style={{display:'flex',gap:8,marginTop:16}}><button style={btnStyle(C.accent,true)} onClick={doSave}>Enregistrer</button><button style={btnStyle(C.red)} onClick={delItem}>Supprimer</button></div>
</Mod>}
...
const rabWidths=[...new Set((data.machines||[]).filter(m=>m.type==='Raboteuse').map(m=>getMachineWidth(m)).filter(Boolean))].sort((a,b)=>Number(b)-Number(a));
...
const setVal=(key,dur,v)=>{const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.forfaits)nd.forfaits={};if(!nd.forfaits[key])nd.forfaits[key]={};nd.forfaits[key][dur]=Number(v)||0;save(nd)};
const prefix=tab;
...
const[tgToken,setTgToken]=useState(data.telegramBotToken||'');
const[tgChatId,setTgChatId]=useState(data.telegramAdminChatId||'');
const[tgStatus,setTgStatus]=useState('');
const[tgStart,setTgStart]=useState(data.tgNotifyStart!==false);
const[tgPause,setTgPause]=useState(data.tgNotifyPause!==false);
const[tgResume,setTgResume]=useState(data.tgNotifyResume!==false);
const[tgDone,setTgDone]=useState(data.tgNotifyDone!==false);
...
<div style={{borderTop:'1px solid #eee',marginTop:16,paddingTop:12}}><h3>Notifications Telegram</h3>
<div style={{fontSize:12,color:C.dim,marginBottom:8,lineHeight:1.6}}>Recois une alerte sur ton telephone (meme app fermee) quand un salarie pointe.<br/><b>1.</b> Dans Telegram, ecris a <b>@BotFather</b> → envoie <b>/newbot</b> → choisis un nom → il te donne un <b>token</b>.<br/><b>2.</b> Colle le token ci-dessous.<br/><b>3.</b> Ouvre ton bot dans Telegram, appuie sur <b>Demarrer</b> et envoie « bonjour ».<br/><b>4.</b> Clique <b>Lier un admin</b> (ouvre le lien sur chaque téléphone de la famille → Démarrer), puis <b>Tester</b>.</div>
<Fl label="Token du bot (ex: 123456789:AAH...)"><input type="password" style={inputStyle} value={tgToken} onChange={e=>setTgToken(e.target.value)} placeholder="123456789:AAH..."/></Fl>
...
<label style={{display:'block',marginBottom:5,cursor:'pointer'}}><input type="checkbox" checked={tgStart} onChange={e=>setTgStart(e.target.checked)} style={{marginRight:8}}/>🟢 embauche</label>
<label style={{display:'block',marginBottom:5,cursor:'pointer'}}><input type="checkbox" checked={tgPause} onChange={e=>setTgPause(e.target.checked)} style={{marginRight:8}}/>⏸ se met en pause</label>
<label style={{display:'block',marginBottom:5,cursor:'pointer'}}><input type="checkbox" checked={tgResume} onChange={e=>setTgResume(e.target.checked)} style={{marginRight:8}}/>▶️ reprend apres la pause</label>
<label style={{display:'block',marginBottom:5,cursor:'pointer'}}><input type="checkbox" checked={tgDone} onChange={e=>setTgDone(e.target.checked)} style={{marginRight:8}}/>🔴 debauche (fin de journee)</label>
...
const[demainOuvert,setDemainOuvert]=useState(false);
...
const openInbox=()=>{setShowInbox(true);if(unreadCount>0){const nd=JSON.parse(JSON.stringify(_liveData||data));(nd.messages||[]).forEach(m=>{if(m.toEmpId===empId&&!m.read)m.read=true});save(nd)}};
...
const setEquipStatus=(eqId,status)=>{const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.machineEquipmentStatus)nd.machineEquipmentStatus={};if(!nd.machineEquipmentStatus[selectedMachineId])nd.machineEquipmentStatus[selectedMachineId]={};nd.machineEquipmentStatus[selectedMachineId][eqId]=status;save(nd)};
...
const _duree=m=>m==null?'—':(m<60?Math.round(m)+' min':_hm(m));
const _mn=(a,b)=>{const[h1,m1]=a.split(':').map(Number);const[h2,m2]=b.split(':').map(Number);let d=(h2*60+m2)-(h1*60+m1);if(d<0)d+=1440;return d};
const _hDe=iso=>{try{const d=new Date(iso);return pad2(d.getHours())+':'+pad2(d.getMinutes())}catch(e){return ''}};
const _enCours=(status!=='off'&&lastEntry&&lastEntry.startTime&&!lastEntry.endTime)?Math.max(0,_mn(lastEntry.startTime,pad2(new Date().getHours())+':'+pad2(new Date().getMinutes()))-(lastEntry.pauseMin||0)):0;
...
const _envoyerManque=(j,quoi)=>{
  ...
  const txt=emp.name+' — '+(j.location||_cli(j))+' ('+(j.billingStart||'')+', '+_cli(j)+') : il me manque '+quoi+'.';
  nd.messages.push({id:uid(),fromEmpId:empId,toEmpId:null,date:new Date().toISOString(),content:txt,read:false,kind:'manque'});
  save(nd);
  setFeuille({type:'envoye',texte:txt});
};
...
<div className="vue" hidden={vue!=='jour'}>
<div className="filigrane" aria-hidden="true"><i></i></div>
<div className="corps">
<div className="barre-id">
<button className="pastille" onClick={()=>setVue('profil')} title="Mon espace">{(emp.name||'?').charAt(0).toUpperCase()}</button>
<div className="qui">{_dateLongue}<small>{_jourJobs.length?_jourJobs.length+(_jourJobs.length>1?' chantiers':' chantier'):'aucun chantier'}</small></div>
<div className="totaux">
<div className="tb"><b>{_hm(_cumulJour)}</b><small>aujourd'hui</small></div>
<div className="tb sem"><b>{_hm(_cumulSem)}</b><small>semaine</small></div>
</div>
</div>
<div className="zonefil">
<div className="fil">
...
<div className="ident">
<div className="client">{_cli(j)}</div>
<div className="machine">{_mach(j)}</div>
{j.forfaitType?<div className="forfait">{j.forfaitType}</div>:null}
</div>
</div>
{!fini&&<div className="contacts">
<button className={'contact'+(j.siteManager?'':' vide')} onClick={()=>j.siteManager?_feuilleChef(j):_feuilleManque(j)}>
<span className="ic">📞</span><span className="txt"><b>{j.siteManager||'Chef non indiqué'}</b><small>{j.siteManager?(j.siteManagerPhone||'numéro manquant'):'personne à appeler'}</small></span></button>
<button className={'contact'+(gps?'':' vide')} onClick={()=>gps?_feuilleGps(j):_feuilleManque(j)}>
<span className="ic">📍</span><span className="txt"><b>{gps?'Itinéraire':'Pas de point GPS'}</b><small>{gps?'point enregistré':'adresse seule'}</small></span></button>
</div>}
...
<div className="dem-liste">
{!demainOuvert&&_demainJobs.map(j=>(
<button key={j.id} className="dl" onClick={()=>setDemainOuvert(true)}>
<b>{j.billingStart||'--:--'}</b><span className="dl-lieu">{j.location||_cli(j)}</span><span className="dl-cli">{_cli(j)}</span>
</button>))}
{demainOuvert&&_demainJobs.map(j=>{const g=_gpsDe(j);return(
<div key={j.id} className="df">
<div className="df-h">{j.billingStart||'--:--'}</div>
<div className="df-txt">
<div className="df-lieu">{j.location||_cli(j)}</div>
<div className="df-sous">{[_cli(j),_mach(j),j.siteManager?'chef '+j.siteManager:'chef non indiqué'].filter(Boolean).join(' · ')}</div>
</div>
<div className="df-act">
<button className={'ico'+(j.siteManagerPhone?'':' vide')} onClick={()=>j.siteManager?_feuilleChef(j):_feuilleManque(j)}>📞</button>
<button className={'ico'+(g?'':' vide')} onClick={()=>g?_feuilleGps(j):_feuilleManque(j)}>📍</button>
<button className={'ico'+((j.siteManager&&j.siteManagerPhone&&g)?'':' alerte')} onClick={()=>_feuilleManque(j)}>⚠</button>
</div>
</div>)})}
</div>
{demainOuvert&&<button className="replier" onClick={()=>setDemainOuvert(false)}>⌄ Réduire</button>}
</div>
<button className="creux heures" onClick={()=>setVue('heures')}>⏱ Mes heures</button>
</div>
</div>

<div className="vue" hidden={vue!=='heures'}>
<div className="tete-vue">
<button className="retour" onClick={()=>setVue('jour')}>←</button>
<h3>Mes heures</h3>
</div>
...
{souci&&<div className="mot-souci">Journée jamais clôturée : la débauche manque. Touche l'heure pour la corriger.</div>}
<div className="chaine">
{ch.map((x,i)=>{
 if(!x.ev)return(<div key={'g'+i} className={'gap '+x.nat}><span className="gap-t">{x.nat}</span><span className="gap-d">{_duree(x.d)}</span></div>);
 const e=x.ev;
 let cls='ev';
 if(e.k==='emb'||e.k==='deb')cls+=' borne';
 if(e.k==='ch2')cls+=' fin';
 if(e.k==='pa1'||e.k==='pa2')cls+=' pa';
 if(e.k==='pa2')cls+=' reprise';
 if(e.k==='ch1')cls+=' arrivee';
 if(x.dans&&e.k!=='pa1'&&e.k!=='pa2')cls+=' dans';
...
<div className="vue" hidden={vue!=='profil'}>
<div className="tete-vue">
<button className="retour" onClick={()=>setVue('jour')}>←</button>
<h3>{emp.name}<span className="sous">{[(emp.role==='employee'||!emp.role)?'chauffeur':emp.role,_mach({machineId:emp.machineId})].filter(Boolean).join(' · ')}</span></h3>
</div>
<div className="liste-jours">
{[
 {t:'Ma journée',items:[
  {ic:'📅',l:'Absence ou rendez-vous',s:'congé, maladie, RTT, formation…',f:()=>{setShowRdv(true);setRdvType('absence')}},
  {ic:'⏱',l:'Saisir mes heures à la main',s:'journée oubliée ou à corriger',f:()=>{setShowManual(true);setManDate(today);setManStart('');setManEnd('');setManPause(0)}}]},
 {t:'Ma machine',items:[
...
<div key={g.t} className="groupe">
<h4>{g.t}</h4>
<div className="menu">
{g.items.map(it=>(
<button key={it.l} className={'item'+(it.sortie?' sortie':'')} onClick={it.f}>
<span className="ic">{it.ic}</span>
<span className="lb">{it.l}{it.s?<small>{it.s}</small>:null}</span>
{it.nb>0?<span className="pastille-nb">{it.nb}</span>:null}
{it.sortie?null:<span className="fl">›</span>}
</button>))}
</div>
</div>))}
</div>
...
<div className={'voile'+(feuille?' on':'')} onClick={_fermerFeuille}></div>
<div className={'feuille'+(feuille?' on':'')}>
<div className="f-poignee"></div>
{feuille&&feuille.type==='chef'&&<React.Fragment>
<div className="f-titre">{feuille.job.siteManager}</div>
<div className="f-sous">{'Chef de chantier · '+(feuille.job.location||_cli(feuille.job))}</div>
{feuille.job.siteManagerPhone
 ?<React.Fragment>
   <div className="f-tel">{feuille.job.siteManagerPhone}</div>
   <div className="f-actions"><div className="f-duo">
   <button onClick={()=>{location.href='tel:'+String(feuille.job.siteManagerPhone).replace(/\s/g,'')}}>📞 Appeler</button>
   <button className="creux" onClick={()=>{location.href='sms:'+String(feuille.job.siteManagerPhone).replace(/\s/g,'')}}>💬 SMS</button>
   </div></div>
  </React.Fragment>
 :<React.Fragment>
   <div className="f-sous">Son numéro n'est pas renseigné dans le planning.</div>
   <div className="f-actions"><button className="manque" onClick={()=>_envoyerManque(feuille.job,'le numéro du chef de chantier')}>⚠ Demander son numéro</button></div>
  </React.Fragment>}
</React.Fragment>}
...
{feuille&&feuille.type==='manque'&&<React.Fragment>
<div className="f-titre">Qu'est-ce qui te manque ?</div>
<div className="f-sous">{(feuille.job.location||_cli(feuille.job))+' · '+_cli(feuille.job)+' · '+(feuille.job.billingStart||'')}</div>
...
<div className="f-sous">L'admin le reçoit dans ses messages.</div>
<div className="f-envoye">{feuille.texte}</div>
<div className="f-actions"><button className="creux" onClick={_fermerFeuille}>Fermer</button></div>
</React.Fragment>}
</div>
...
const toDecHours=(startTime,endTime,pauseMin)=>{if(!startTime||!endTime)return 0;const[sh,sm]=(startTime||'0:0').split(':').map(Number);const[eh,em]=(endTime||'0:0').split(':').map(Number);let mins=(eh*60+em)-(sh*60+sm);if(mins<0)mins+=24*60;if(mins===0&&(pauseMin||0)>0)mins=24*60;mins-=(pauseMin||0);return Math.round(Math.max(0,mins/60)*100)/100};
...
if(!editable){trs.push(<tr key={r.date} style={{background:bg}}><td style={tdS}>{r.horodateur||''}</td><td style={tdS}>{r.week}</td>{dateCell}<td style={{...tdS,color:r.absence?C.red:C.dim}}>{r.absence||''}</td><td style={tdS}>{r.start}{r.nb>1&&<div style={{fontSize:9,color:C.dim,whiteSpace:'nowrap'}}>{r.nb} pointages : {r.segments}</div>}</td><td style={tdS}>{r.breakStart}</td><td style={{...tdS,fontWeight:600,color:r.meal==='PANIER'?C.accent:C.orange}}>{r.meal}</td><td style={tdS}>{r.breakEnd}</td><td style={tdS}>{r.end}{r.endJ1&&<span style={{fontSize:9,color:C.dim}}> +1</span>}</td><td style={{...tdS,fontWeight:600}}>{fmtDec(r.worked)}</td><td style={tdS}>{fmtDec(r.ref||0)} <span style={{color:C.purple}}>{fmtDec(r.night||0)}</span></td></tr>)}else{
...
return(
<div>
<h2 style={{marginBottom:4}}>📋 Recap heures — tous les chauffeurs <span style={{fontSize:10,color:C.dim,fontWeight:400,marginLeft:8}}>v2026.09.24-6</span></h2>
...
const addStation=()=>{if(!newStationName.trim()){alert('Nom requis');return}const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.stations)nd.stations=[];const id=uid();nd.stations.push({id,name:newStationName.trim(),type:newStationType,location:newStationLocation.trim(),createdAt:new Date().toISOString(),_updatedAt:Date.now()});if(newStationCopyFrom){if(!nd.stationProducts)nd.stationProducts=[];const sourceProds=(nd.stationProducts||[]).filter(p=>p.stationId===newStationCopyFrom);sourceProds.forEach(sp=>{nd.stationProducts.push({...sp,id:uid(),stationId:id,quantity:0,history:[],_updatedAt:Date.now()})})}save(nd);setSelStationId(id);setShowAddStation(false);setNewStationName('');setNewStationLocation('');setNewStationType('station');setNewStationCopyFrom('')};
const delStation=()=>{if(!selStation||!confirm('Supprimer "'+selStation.name+'" et tous ses produits ?'))return;const nd=JSON.parse(JSON.stringify(_liveData||data));nd.stations=(nd.stations||[]).filter(s=>s.id!==selStation.id);nd.stationProducts=(nd.stationProducts||[]).filter(p=>p.stationId!==selStation.id);nd.stationMovements=(nd.stationMovements||[]).filter(m=>m.stationId!==selStation.id);save(nd);const rest=(nd.stations||[]);setSelStationId(rest[0]?rest[0].id:'')};
const addProductOpen=()=>setEditProduct({id:null,stationId:selStationId,name:'',unit:'bidon',category:'piste',color:'#0891b2',quantity:0,unitPrice:0,minStock:1,supplier:''});
const saveProduct=()=>{if(!editProduct.name.trim()){alert('Nom requis');return}const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.stationProducts)nd.stationProducts=[];if(editProduct.id){const idx=nd.stationProducts.findIndex(p=>p.id===editProduct.id);if(idx>=0)nd.stationProducts[idx]={...editProduct,name:editProduct.name.trim(),category:editProduct.category||'piste',quantity:Number(editProduct.quantity)||0,unitPrice:Number(editProduct.unitPrice)||0,minStock:Number(editProduct.minStock)||0,_updatedAt:Date.now()}}else{nd.stationProducts.push({...editProduct,id:uid(),name:editProduct.name.trim(),category:editProduct.category||'piste',quantity:Number(editProduct.quantity)||0,unitPrice:Number(editProduct.unitPrice)||0,minStock:Number(editProduct.minStock)||0,history:[],_updatedAt:Date.now()})}save(nd);setEditProduct(null)};
const delProduct=(pid)=>{if(!confirm('Supprimer ce produit ?'))return;const nd=JSON.parse(JSON.stringify(_liveData||data));nd.stationProducts=(nd.stationProducts||[]).filter(p=>p.id!==pid);save(nd)};
const moveStock=(p,type,askQty)=>{let n=1;if(askQty){const qte=prompt((type==='in'?'Quantité à ajouter ?':'Quantité à retirer ?'),'1');if(!qte)return;n=Number(qte);if(!n||n<=0)return}if(type==='out'&&n>p.quantity){alert('Stock insuffisant');return}const nd=JSON.parse(JSON.stringify(_liveData||data));const pp=nd.stationProducts.find(x=>x.id===p.id);if(pp){pp.quantity=type==='in'?(pp.quantity||0)+n:Math.max(0,(pp.quantity||0)-n);pp._updatedAt=Date.now();if(!nd.stationMovements)nd.stationMovements=[];nd.stationMovements.unshift({id:uid(),stationId:p.stationId,productId:p.id,type,qty:n,unitPrice:pp.unitPrice||0,date:new Date().toISOString(),userId:'admin',reason:''});nd.stationMovements=nd.stationMovements.slice(0,1000);save(nd)}};
const openTransfer=(p)=>{setTransferProduct(p);setTransferDest('');setTransferQty('')};
const doTransfer=()=>{if(!transferProduct||!transferDest){alert('Choisir une destination');return}const n=Number(transferQty);if(!n||n<=0){alert('Quantite invalide');return}if(n>transferProduct.quantity){alert('Stock insuffisant');return}const nd=JSON.parse(JSON.stringify(_liveData||data));const srcP=nd.stationProducts.find(p=>p.id===transferProduct.id);if(!srcP)return;srcP.quantity=Math.max(0,(srcP.quantity||0)-n);srcP._updatedAt=Date.now();const destProd=(nd.stationProducts||[]).find(p=>p.stationId===transferDest&&p.name.toLowerCase()===transferProduct.name.toLowerCase()&&(p.category||'piste')===(transferProduct.category||'piste')&&(p.unit||'')===(transferProduct.unit||''));let destProductId;if(destProd){destProd.quantity=(destProd.quantity||0)+n;destProd._updatedAt=Date.now();destProductId=destProd.id}else{const newId=uid();nd.stationProducts.push({...transferProduct,id:newId,stationId:transferDest,quantity:n,history:[],_updatedAt:Date.now()});destProductId=newId}if(!nd.stationMovements)nd.stationMovements=[];const now=new Date().toISOString();nd.stationMovements.unshift({id:uid(),stationId:transferProduct.stationId,productId:transferProduct.id,type:'transfer_out',qty:n,unitPrice:transferProduct.unitPrice||0,destStationId:transferDest,date:now,userId:'admin'});nd.stationMovements.unshift({id:uid(),stationId:transferDest,productId:destProductId,type:'transfer_in',qty:n,unitPrice:transferProduct.unitPrice||0,sourceStationId:transferProduct.stationId,date:now,userId:'admin'});nd.stationMovements=nd.stationMovements.slice(0,1000);save(nd);setTransferProduct(null);setTransferDest('');setTransferQty('')};
...
{/* Tableau produits */}
{stationProducts.length===0?<div style={{padding:24,textAlign:'center',background:'#f8fafc',borderRadius:10,border:'1px dashed '+C.border,color:C.dim}}>Aucun produit dans cette station.<br/>Cliquez sur « + Produit » pour en ajouter.</div>:(()=>{const cats=[{k:'piste',l:'🛣 Produits piste',color:'#0891b2'},{k:'rouleaux',l:'🌀 Produits rouleaux',color:'#7c3aed'}];const rowFor=p=>{const low=p.minStock>0&&p.quantity<=p.minStock;return(<tr key={p.id} style={{borderTop:'1px solid '+C.border,background:low?'#fef2f2':'#fff'}}>
<td style={{padding:'10px'}}><div style={{display:'flex',alignItems:'center',gap:8}}>{p.color&&<div style={{width:16,height:16,borderRadius:'50%',background:p.color,border:'2px solid #fff',boxShadow:'0 0 0 1px '+C.border,flexShrink:0}} title={p.color}/>}<div><div style={{fontWeight:700}}>{p.name}</div>{p.supplier?<div style={{fontSize:11,color:C.dim,fontWeight:400}}>{p.supplier}</div>:null}</div></div></td>
...
<button onClick={()=>moveStock(p,'in')} onContextMenu={e=>{e.preventDefault();moveStock(p,'in',true)}} style={{background:'#dcfce7',border:'1px solid #16a34a',color:'#15803d',padding:'4px 10px',borderRadius:6,cursor:'pointer',fontWeight:700,fontSize:13,marginRight:4}} title="Entree +1 (clic droit pour saisir une quantite)">+</button>
<button onClick={()=>moveStock(p,'out')} onContextMenu={e=>{e.preventDefault();moveStock(p,'out',true)}} style={{background:'#fee2e2',border:'1px solid #dc2626',color:'#991b1b',padding:'4px 10px',borderRadius:6,cursor:'pointer',fontWeight:700,fontSize:13,marginRight:4}} title="Sortie -1 (clic droit pour saisir une quantite)">−</button>
<button onClick={()=>openTransfer(p)} style={{background:'#eff6ff',border:'1px solid '+ELEPHANT_BLUE,color:ELEPHANT_BLUE,padding:'4px 10px',borderRadius:6,cursor:'pointer',fontWeight:700,fontSize:13,marginRight:4}} title="Transferer vers une autre station">↗</button>
<button onClick={()=>setEditProduct({...p,category:p.category||'piste'})} style={{background:'#fff',border:'1px solid '+C.dim,color:C.dim,padding:'4px 8px',borderRadius:6,cursor:'pointer',fontSize:11,marginRight:4}} title="Editer">✎</button>
...
{cats.map(cat=>{const list=stationProducts.filter(p=>(p.category||'piste')===cat.k);if(list.length===0)return null;const catTotal=list.reduce((s,p)=>s+(p.quantity||0)*(p.unitPrice||0),0);return(<div key={cat.k} style={{background:'#fff',borderRadius:10,border:'2px solid '+cat.color+'40',overflow:'hidden'}}>
...
<th style={{padding:'8px',textAlign:'left',color:C.dim,fontWeight:700,fontSize:11,textTransform:'uppercase'}}>Produit</th>
<th style={{padding:'8px',textAlign:'right',color:C.dim,fontWeight:700,fontSize:11,textTransform:'uppercase'}}>Stock</th>
<th style={{padding:'8px',textAlign:'center',color:C.dim,fontWeight:700,fontSize:11,textTransform:'uppercase'}}>Unite</th>
<th style={{padding:'8px',textAlign:'right',color:C.dim,fontWeight:700,fontSize:11,textTransform:'uppercase'}}>Prix U.</th>
<th style={{padding:'8px',textAlign:'right',color:C.dim,fontWeight:700,fontSize:11,textTransform:'uppercase'}}>Valeur</th>
<th style={{padding:'8px',textAlign:'right',color:C.dim,fontWeight:700,fontSize:11,textTransform:'uppercase'}}>Min</th>
<th style={{padding:'8px',textAlign:'center',color:C.dim,fontWeight:700,fontSize:11,textTransform:'uppercase'}}>Actions</th>
</tr></thead>
...
<tbody>{stationMovs.map(m=>{const prod=products.find(p=>p.id===m.productId);const otherSta=stations.find(s=>s.id===(m.type==='transfer_out'?m.destStationId:m.sourceStationId));let label,bg,fg,sign;if(m.type==='in'){label='+ Entree';bg='#dcfce7';fg='#15803d';sign='+'}else if(m.type==='out'){label='− Sortie';bg='#fee2e2';fg='#991b1b';sign='-'}else if(m.type==='transfer_out'){label='↗ Transfert → '+(otherSta?otherSta.name:'?');bg='#fef3c7';fg='#9a3412';sign='-'}else if(m.type==='transfer_in'){label='↘ Transfert ← '+(otherSta?otherSta.name:'?');bg='#dbeafe';fg='#1d4ed8';sign='+'}else{label=m.type;bg='#f1f5f9';fg=C.dim;sign=''}return(<tr key={m.id} style={{borderTop:'1px solid '+C.border}}><td style={{padding:'8px',color:C.dim,fontSize:12}}>{new Date(m.date).toLocaleString('fr-FR',{dateStyle:'short',timeStyle:'short'})}</td><td style={{padding:'8px',fontWeight:600}}>{prod?prod.name:'(supprimé)'}</td><td style={{padding:'8px',textAlign:'center'}}><span style={{background:bg,color:fg,padding:'3px 10px',borderRadius:6,fontSize:11,fontWeight:700,whiteSpace:'nowrap'}}>{label}</span></td><td style={{padding:'8px',textAlign:'right',fontWeight:800,color:fg}}>{sign}{m.qty}</td><td style={{padding:'8px',textAlign:'right'}}>{fmtMoney((m.qty||0)*(m.unitPrice||0))}</td></tr>)})}</tbody></table>}
</div>}
{/* Modal ajout station */}
...
<Fl label="Categorie"><div style={{display:'flex',gap:8}}>
<button onClick={()=>setEditProduct({...editProduct,category:'piste'})} style={{flex:1,padding:'10px',borderRadius:8,border:'2px solid #0891b2',background:(editProduct.category||'piste')==='piste'?'#0891b2':'#fff',color:(editProduct.category||'piste')==='piste'?'#fff':'#0891b2',fontWeight:700,cursor:'pointer'}}>🛣 Piste</button>
<button onClick={()=>setEditProduct({...editProduct,category:'rouleaux'})} style={{flex:1,padding:'10px',borderRadius:8,border:'2px solid #7c3aed',background:editProduct.category==='rouleaux'?'#7c3aed':'#fff',color:editProduct.category==='rouleaux'?'#fff':'#7c3aed',fontWeight:700,cursor:'pointer'}}>🌀 Rouleaux</button>
</div></Fl>
...
<Fl label="Couleur du produit"><div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
<input type="color" value={editProduct.color||'#0891b2'} onChange={e=>setEditProduct({...editProduct,color:e.target.value})} style={{width:48,height:40,border:'2px solid '+C.border,borderRadius:8,cursor:'pointer',padding:2,background:'#fff'}}/>
<div style={{display:'flex',gap:6,flexWrap:'wrap',flex:1}}>
{['#3b82f6','#06b6d4','#16a34a','#65a30d','#eab308','#f97316','#dc2626','#ec4899','#7c3aed','#a16207','#1e293b','#94a3b8'].map(c=>(<button key={c} onClick={()=>setEditProduct({...editProduct,color:c})} style={{width:30,height:30,borderRadius:'50%',background:c,border:editProduct.color===c?'3px solid '+C.text:'2px solid #fff',boxShadow:editProduct.color===c?'0 0 0 1px '+C.text:'0 0 0 1px '+C.border,cursor:'pointer',padding:0}} title={c}/>))}
</div>
</div></Fl>
...
<Fl label="Unite"><input style={inputStyle} value={editProduct.unit} onChange={e=>setEditProduct({...editProduct,unit:e.target.value})} placeholder="bidon, L, kg, p..."/><div style={{display:'flex',gap:4,marginTop:6,flexWrap:'wrap'}}>{['bidon','L','kg','piece','seau','carton'].map(u=><button key={u} onClick={()=>setEditProduct({...editProduct,unit:u})} style={{padding:'4px 10px',borderRadius:6,border:'1px solid '+(editProduct.unit===u?ELEPHANT_BLUE:C.border),background:editProduct.unit===u?ELEPHANT_BLUE:'#fff',color:editProduct.unit===u?'#fff':C.dim,cursor:'pointer',fontSize:12,fontWeight:600}}>{u}</button>)}</div></Fl>
</div>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
...
<div style={{display:'flex',alignItems:'center',gap:8}}>{transferProduct.color&&<div style={{width:14,height:14,borderRadius:'50%',background:transferProduct.color,border:'2px solid #fff',boxShadow:'0 0 0 1px '+C.border}}/>}<div style={{fontWeight:800,fontSize:15}}>{transferProduct.name}</div></div>
<div style={{fontSize:12,color:C.dim,marginTop:4}}>{selStation?selStation.name:''} • Stock actuel : <b style={{color:ELEPHANT_BLUE}}>{transferProduct.quantity} {transferProduct.unit}</b></div>
</div>
<Fl label="Destination"><select style={inputStyle} value={transferDest} onChange={e=>setTransferDest(e.target.value)}>
<option value="">-- Choisir une station ou un depot --</option>
...
const stationProducts=products.filter(p=>p.stationId===selStationId);
const stationMovs=movements.filter(m=>m.stationId===selStationId).slice(0,50);
const moveStock=(p,type,askQty)=>{let n=1;if(askQty){const qte=prompt(type==='in'?'Quantite a ajouter ?':'Quantite a retirer ?','1');if(!qte)return;n=Number(qte);if(!n||n<=0)return}if(type==='out'&&n>p.quantity){alert('Stock insuffisant');return}const nd=JSON.parse(JSON.stringify(_liveData||data));const pp=nd.stationProducts.find(x=>x.id===p.id);if(pp){pp.quantity=type==='in'?(pp.quantity||0)+n:Math.max(0,(pp.quantity||0)-n);pp._updatedAt=Date.now();if(!nd.stationMovements)nd.stationMovements=[];nd.stationMovements.unshift({id:uid(),stationId:p.stationId,productId:p.id,type,qty:n,unitPrice:pp.unitPrice||0,date:new Date().toISOString(),userId:user.id});nd.stationMovements=nd.stationMovements.slice(0,1000);save(nd)}};
const openTransfer=(p)=>{setTransferProduct(p);setTransferDest('');setTransferQty('')};
...
{cats.map(cat=>{const list=stationProducts.filter(p=>(p.category||'piste')===cat.k);if(list.length===0)return null;return(<div key={cat.k} style={{background:'#fff',borderRadius:10,border:'2px solid '+cat.color+'40',overflow:'hidden',marginBottom:12}}>
<div style={{padding:'10px 14px',background:cat.color+'12',borderBottom:'1px solid '+cat.color+'30',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
<div style={{color:cat.color,fontWeight:800,fontSize:14,textTransform:'uppercase',letterSpacing:'0.5px'}}>{cat.l}</div>
<div style={{fontSize:12,color:C.dim,fontWeight:600}}>{list.length} produit(s)</div>
</div>
...
<div style={{display:'flex',gap:6}}>
<button onClick={()=>moveStock(p,'in')} onContextMenu={e=>{e.preventDefault();moveStock(p,'in',true)}} style={{background:'#dcfce7',border:'2px solid #16a34a',color:'#15803d',padding:'10px 16px',borderRadius:8,cursor:'pointer',fontWeight:800,fontSize:18}} title="Entree +1 (clic long / clic droit pour saisir une quantite)">+</button>
<button onClick={()=>moveStock(p,'out')} onContextMenu={e=>{e.preventDefault();moveStock(p,'out',true)}} style={{background:'#fee2e2',border:'2px solid #dc2626',color:'#991b1b',padding:'10px 16px',borderRadius:8,cursor:'pointer',fontWeight:800,fontSize:18}} title="Sortie -1 (clic long / clic droit pour saisir une quantite)">−</button>
...
Deux notions differentes : le « temps passe sur le chantier » vient de la fin de chantier pointee par le chauffeur (champs fin_de_chantier_pointee et duree_sur_chantier dans planning, ou fins_de_chantier dans pointages) ; les « heures de la journee » (embauche, debauche, pauses) viennent de pointages. Si on te demande le temps sur un chantier, reponds avec la duree de fin de chantier, meme si le chauffeur n'a pas encore debauche. Donne les durees en heures et minutes (« 2 heures 01 »), les heures en heures et minutes.
...
const fields=[
 ...
 {k:'chantierFont',l:'Cases : taille du texte du lieu / chantier',min:10,max:24,step:1,unit:'px'},
 {k:'locWidth',l:'Cases : largeur de la case Lieu / adresse',min:120,max:400,step:10,unit:'px'},
 {k:'rowGap',l:'Cases : ecart entre les cases d\'une ligne',min:0,max:20,step:1,unit:'px'},
 {k:'rowPadV',l:'Cases : hauteur des lignes de chantier',min:2,max:18,step:1,unit:'px'},
 {k:'chipFont',l:'Cases : taille des pastilles d\'heures',min:9,max:20,step:1,unit:'px'},
];
...
const doSave=useCallback(async nd=>{const prev=dataRef.current;stampUpdatedAt(prev&&prev.jobs,nd&&nd.jobs);_checkStockZero(prev,nd);savesInProgress.current++;savingRef.current=true;undoStack.current=[...(undoStack.current||[]).slice(-19),JSON.stringify(prev)];setData(nd);dataRef.current=nd;_liveData=nd;// la synchro des pointages ne doit JAMAIS empecher l'enregistrement des donnees :
// un seul try/catch commun faisait perdre en silence la modification en cours.
try{await teSyncChanges(prev,nd)}catch(e){console.warn('synchro pointages',e)}
if(nd&&nd._teDeletes)delete nd._teDeletes;
try{await saveData(nd)}catch(e){console.warn('save error',e);try{localSave(nd)}catch(_){}}finally{setTimeout(()=>{savesInProgress.current=Math.max(0,savesInProgress.current-1);if(savesInProgress.current===0)savingRef.current=false},5000)}},[]);
