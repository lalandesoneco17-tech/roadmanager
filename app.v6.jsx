...
<Coche on={!!j.paid} color={C.purple} label="payé" title="Le client a payé" onClick={()=>modif(j.id,jj=>{jj.paid=!jj.paid})}/>
...
<Coche on={false} color={C.purple} label="payé"/><Coche on={false} color="#ca8a04" label="bon"/>
{empId&&<span style={{marginLeft:'auto',display:'flex',gap:4}}><button onClick={()=>{setDepotFormEmpId(empId);setShowDepotForm(true)}} style={{background:'#64748b',color:'#fff',border:'none',borderRadius:4,padding:'2px 8px',cursor:'pointer',fontSize:11}}>Dépôt</button><button onClick={()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.jobs)nd.jobs=[];nd.jobs.push({id:uid(),date:selDate,employeeId:empId,type:'depot',depotActivity:'Repos',rest:true});save(nd)}} style={{background:C.red,color:'#fff',border:'none',borderRadius:4,padding:'2px 8px',cursor:'pointer',fontSize:11}}>Repos</button></span>}
</div>);
