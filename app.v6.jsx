...
setTimeout(()=>{const nd2=JSON.parse(JSON.stringify(data));const jj2=nd2.jobs.find(x=>x.id===j.id);if(!jj2)return;const empJobs=(nd2.jobs||[]).filter(jb=>jb.employeeId===jj2.employeeId&&jb.date===jj2.date&&jb.type!=='depot').sort((a,b)=>(a.billingStart||'99:99').localeCompare(b.billingStart||'99:99'));const myIdx=empJobs.findIndex(jb=>jb.id===jj2.id);const prevJob=myIdx>0?empJobs[myIdx-1]:null;const nextJob=myIdx>=0&&myIdx<empJobs.length-1?empJobs[myIdx+1]:null;const depotIds=new Set(['home',...(nd2.depots||[]).map(d=>d.id)]);const isDepotStart=!jj2.startFrom||depotIds.has(jj2.startFrom);const isDepotEnd=!jj2.endAt||depotIds.has(jj2.endAt);let changed=false;if(isDepotStart){const prevGps=prevJob?parseCoords(prevJob.gps||prevJob._geocodedGps):null;if(prevGps&&jj2.startFrom!==prevJob.id){const km=+(haversine(prevGps,gpsJ)*1.3).toFixed(0);jj2.startFrom=prevJob.id;jj2.kmAller=km;jj2.travelMinAller=Math.round((km/80)*60);changed=true}else if(!prevGps&&!jj2.startFrom&&shortDep){jj2.startFrom=shortDep.id;jj2.kmAller=shortDep.km;jj2.travelMinAller=Math.round((shortDep.km/80)*60);changed=true}}if(isDepotEnd){const nextGps=nextJob?parseCoords(nextJob.gps||nextJob._geocodedGps):null;if(nextGps&&jj2.endAt!==nextJob.id){const km=+(haversine(gpsJ,nextGps)*1.3).toFixed(0);jj2.endAt=nextJob.id;jj2.kmRetour=km;jj2.travelMinRetour=Math.round((km/80)*60);changed=true}else if(!nextGps&&!jj2.endAt&&shortArr){jj2.endAt=shortArr.id;jj2.kmRetour=shortArr.km;jj2.travelMinRetour=Math.round((shortArr.km/80)*60);changed=true}}if(changed){jj2.distanceKm=(jj2.kmAller||0)+(jj2.kmRetour||0);jj2.travelMin=(jj2.travelMinAller||0)+(jj2.travelMinRetour||0);save(nd2)}},0)
return null;
})()}
...
{(()=>{
// Ajustement 'Mise sec.' (segs[6]) : si la pause repas tombe APRES l'arrivee depot, on retire la pause du temps depot.
...
const segsAdj=segs.map((sg,i)=>{if(i!==6||!sg)return sg;const a={...sg};if(R['aD']!=null&&_pStart!=null&&_pStart>=R['aD']&&a.tR!=null){a.tR=Math.max(0,a.tR-_pauseM);a.sR=(a.tR/60)*hourly}if(T['aD']!=null&&_pStart!=null&&_pStart>=T['aD']&&a.tT!=null){a.tT=Math.max(0,a.tT-_pauseM);a.sT=(a.tT/60)*hourly}return a});
const agg=(idxs)=>{const r={tT:0,tR:0,sT:0,sR:0,cT:0,cR:0,ccT:0,ccR:0};idxs.forEach(i=>{const sg=segsAdj[i];if(!sg)return;if(sg.tT!=null&&sg.tT>=0)r.tT+=sg.tT;if(sg.tR!=null&&sg.tR>=0)r.tR+=sg.tR;if(sg.sT!=null)r.sT+=sg.sT;if(sg.sR!=null)r.sR+=sg.sR;if(sg.cT!=null)r.cT+=sg.cT;if(sg.cR!=null)r.cR+=sg.cR;if(sg.ccT!=null)r.ccT+=sg.ccT;if(sg.ccR!=null)r.ccR+=sg.ccR});return r};
let _tr=agg([1,5]),_ch=agg([2,3,4]),_dp=agg([0,6]);
// Plafond 8h (480 min) sur le total des 3 carres : on reduit le Depot si depassement
const _max=480;
const _totR=_tr.tR+_ch.tR+_dp.tR;if(_totR>_max){const ex=_totR-_max;_dp.tR=Math.max(0,_dp.tR-ex);_dp.sR=(_dp.tR/60)*hourly}
const _totT=_tr.tT+_ch.tT+_dp.tT;if(_totT>_max){const ex=_totT-_max;_dp.tT=Math.max(0,_dp.tT-ex);_dp.sT=(_dp.tT/60)*hourly}
const blocks=[
  {lbl:'Transfert',d:_tr,color:'#d97706'},
  {lbl:'Chantier',d:_ch,color:'#16a34a'},
  {lbl:'Depot',d:_dp,color:'#0891b2'}
];
...
<div style={{fontWeight:800,color:b.color,marginBottom:6,textAlign:'center',fontSize:13,textTransform:'uppercase',letterSpacing:'0.5px'}}>{b.lbl}</div>
