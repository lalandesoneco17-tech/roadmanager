...
save(nd);
// Notification Telegram a l'admin : chantier signe (nom du chantier + temps passe)
try{const cfg=_liveData||data;const tok=cfg.telegramBotToken,cid=cfg.telegramAdminChatId;if(tok&&cid&&cfg.tgNotifySign!==false){const who=(emp&&emp.name)||'Un salarie';const loc=signJob.location||jj.location||'chantier';const cln=jj.clientId?((nd.clients.find(c=>c.id===jj.clientId)||{}).name||''):'';const dm=jj.signature.durationMin;const dur=dm!=null?Math.floor(dm/60)+'h'+String(dm%60).padStart(2,'0'):'—';const ff=jj.signature.autoForfait?(' • Forfait '+jj.signature.autoForfait):'';tgSend(tok,cid,'✍️ '+who+' — signature validée\n📍 '+loc+(cln?' ('+cln+')':'')+'\n⏱ Temps passé : '+dur+ff)}}catch(e){}
