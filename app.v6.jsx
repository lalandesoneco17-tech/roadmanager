...
const delTE=(tid)=>{if(!confirm('Supprimer ?'))return;const nd=JSON.parse(JSON.stringify(_liveData||data));nd.timeEntries=nd.timeEntries.filter(t=>t.id!==tid);save(nd)};
...
const doSave=useCallback(async nd=>{const prev=dataRef.current;stampUpdatedAt(prev&&prev.jobs,nd&&nd.jobs);_checkStockZero(prev,nd);savesInProgress.current++;savingRef.current=true;undoStack.current=[...(undoStack.current||[]).slice(-19),JSON.stringify(prev)];setData(nd);dataRef.current=nd;_liveData=nd;try{await teSyncChanges(prev,nd);await saveData(nd)}catch(e){console.warn('save error',e)}finally{setTimeout(()=>{savesInProgress.current=Math.max(0,savesInProgress.current-1);if(savesInProgress.current===0)savingRef.current=false},5000)}},[]);
