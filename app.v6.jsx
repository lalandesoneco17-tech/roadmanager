...
// Envoie au chauffeur le planning CONFIRME (definitif) du jour affiche (selDate), via Telegram. Declenche par le bouton 📨 sur sa carte.
const sendConfirmedPlan=async(eId,empNm)=>{const cfg=_liveData||data;const tok=cfg.telegramBotToken;const link=(cfg.telegramEmpChats||{})[eId];if(!tok){alert('Le bot Telegram n est pas configure (Reglages).');return}if(!link||!link.chatId){alert(empNm+" n'a pas encore lie son Telegram.");return}const dayJobs=(cfg.jobs||[]).filter(x=>x.employeeId===eId&&x.date===selDate).sort((a,b)=>String(a.billingStart||'').localeCompare(String(b.billingStart||'')));const dl=fmtDate(new Date(selDate+'T12:00:00'));if(!confirm('Envoyer le planning CONFIRME du '+dl+' a '+empNm+' ?'))return;const prenom=empNm?empNm.split(' ')[0]:'';const lines=['👋 Salut '+prenom+' !','✅ Ton planning est CONFIRME — '+dl+(dayJobs.length?' :':'')];if(!dayJobs.length){lines.push('Rien de prevu pour toi ce jour-la — repos 😴')}else{dayJobs.forEach(jb=>{if(jb.type==='depot'){lines.push('• 🏭 depot'+(jb.depotActivity?' ('+jb.depotActivity+')':''));return}if(jb.type==='repos'||jb.rest){lines.push('• 😴 repos');return}const c=(cfg.clients||[]).find(x=>x.id===jb.clientId);const mc=(cfg.machines||[]).find(x=>x.id===jb.machineId);lines.push('• '+(jb.billingStart||'')+' '+(jb.location||(c?c.name:'chantier'))+(c&&jb.location?' ('+c.name+')':'')+(mc?' ['+mc.name+']':''))})}const r=await tgSend(tok,link.chatId,lines.join('\n'));alert(r&&r.ok?'✅ Planning confirme envoye a '+empNm:'❌ '+((r&&r.error)||'Erreur Telegram'))};
...
<button onClick={e=>{e.stopPropagation();sendConfirmedPlan(eId,emp.name)}} title={((data.telegramEmpChats||{})[eId])?('Envoyer le planning confirme du jour a '+emp.name):(emp.name+' n a pas encore lie son Telegram')} style={{background:((data.telegramEmpChats||{})[eId])?'#0891b2':'#cbd5e1',color:'#fff',border:'none',borderRadius:4,width:20,height:20,cursor:'pointer',fontSize:12,fontWeight:700,lineHeight:'18px',padding:0,flexShrink:0}}>📨</button>
</div>
...
<div style={{marginLeft:'auto',display:'flex',gap:4,alignItems:'center'}}>
<button onClick={()=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.sent=!jj.sent;save(nd)}}} title={j.sent?'Chantier envoye au chauffeur':'Envoyer chantier au chauffeur'} style={{width:24,height:24,borderRadius:6,border:'2px solid '+(j.sent?C.green:C.muted),background:j.sent?C.green:'transparent',color:'#fff',cursor:'pointer',fontSize:14,fontWeight:800,padding:0,lineHeight:1,flexShrink:0}}>{j.sent?'✓':''}</button>
