...
<button onClick={async()=>{const tok=(tgToken||'').trim();if(!tok){setTgStatus('⚠ Colle d abord le token du bot');return}setTgStatus('Recherche de ton message Telegram...');const r=await tgGetChatId(tok);if(r.ok){setTgChatId(r.chatId);save({...data,telegramBotToken:tok,telegramAdminChatId:r.chatId});setTgStatus('✅ Lie a '+(r.name||r.chatId)+' — clique Tester')}else{setTgStatus('❌ '+r.error)}}} style={btnStyle('#0891b2')}>🔗 Lier mon Telegram</button>
<button onClick={async()=>{const tok=(tgToken||'').trim();const cid=(tgChatId||'').trim();if(!tok||!cid){setTgStatus('⚠ Lie d abord ton Telegram (token + bouton Lier)');return}setTgStatus('Envoi du test...');const r=await tgSend(tok,cid,'✅ Test RoadManager — les notifications Telegram fonctionnent !');setTgStatus(r.ok?'✅ Message envoye ! Regarde Telegram.':'❌ '+r.error)}} style={btnStyle(C.green)}>✉ Tester</button>
</div>
{tgChatId&&<div style={{fontSize:12,color:C.green}}>✓ Compte lie (ID {tgChatId})</div>}
...
try{const cfg=_liveData||data;const tok=cfg.telegramBotToken,cid=cfg.telegramAdminChatId,who=(emp&&emp.name)||'Un salarie';const msgs={start:cfg.tgNotifyStart!==false?'🟢 '+who+' a embauché à '+time:null,pause_start:cfg.tgNotifyPause!==false?'⏸ '+who+' en pause à '+time:null,resume:cfg.tgNotifyResume!==false?'▶️ '+who+' a repris à '+time:null,done:cfg.tgNotifyDone!==false?'🔴 '+who+' a débauché à '+time:null};const m=msgs[type];if(tok&&cid&&m)tgSend(tok,cid,m)}catch(e){}};
...
(async()=>{try{const cfg=_liveData||data;const tok=cfg.telegramBotToken,cid=cfg.telegramAdminChatId;if(!tok||!cid||cfg.tgNotifySign===false)return;const who=(emp&&emp.name)||'Un salarie';const loc=signJob.location||jj.location||'chantier';const cln=jj.clientId?((nd.clients.find(c=>c.id===jj.clientId)||{}).name||''):'';const dm=jj.signature.durationMin;const dur=dm!=null?Math.floor(dm/60)+'h'+String(dm%60).padStart(2,'0'):'—';const ff=jj.signature.autoForfait?(' • Forfait '+jj.signature.autoForfait):'';const lines=['✍️ '+who+' — signature validée','📍 '+loc+(cln?' ('+cln+')':''),'⏱ Temps passé : '+dur+ff];const jc=parseCoords(signJob.gps||signJob._geocodedGps);if(jc){let best=null;(cfg.depots||[]).forEach(d=>{const dc=d._coords?parseCoords(typeof d._coords==='string'?d._coords:d._coords.join(',')):null;if(dc){const km=haversine(jc,dc);if(!best||km<best.km)best={d,dc}}});if(best){const r=await osmRoute(jc,best.dc);lines.push('🏭 Retour dépôt '+(best.d.name||'')+' : '+r.min+' min ('+r.km+' km)')}const hc=getEmpCoords(cfg,empId);if(hc){const r=await osmRoute(jc,hc);lines.push('🏠 Retour domicile : '+r.min+' min ('+r.km+' km)')}}else{lines.push('🚗 Trajet : (pas de GPS sur le chantier)')}const tmr=new Date();tmr.setDate(tmr.getDate()+1);const tISO=fmtDateISO(tmr);const next=(cfg.jobs||[]).filter(j=>j.employeeId===empId&&j.date===tISO);if(next.length){lines.push('📅 Demain :');next.forEach(j=>{const c=(cfg.clients||[]).find(x=>x.id===j.clientId);const mc=(cfg.machines||[]).find(x=>x.id===j.machineId);lines.push('  • '+(j.billingStart||'')+' '+(j.location||(c?c.name:'chantier'))+(c&&j.location?' ('+c.name+')':'')+(mc?' ['+mc.name+']':''))})}else{lines.push('📅 Demain : rien de prévu')}tgSend(tok,cid,lines.join('\n'),{reply_markup:{inline_keyboard:[[{text:'✅ '+who+' peut rentrer au dépôt',callback_data:'rentrer:'+empId}],[{text:'📅 Lui envoyer son planning de demain',callback_data:'plan:'+empId}]]}})}catch(e){}})();
...
<button onClick={async()=>{const tok=(data.telegramBotToken||'');if(!tok){alert('Le bot Telegram n est pas encore configure par l administrateur.');return}const me=await tgGetMe(tok);if(!me.ok){alert('Bot Telegram introuvable : '+me.error);return}window.open('https://t.me/'+me.username+'?start=emp_'+empId,'_blank')}} title={emp.telegramChatId?'Telegram lié ✓':'Lier mon Telegram pour recevoir les messages'} style={{background:emp.telegramChatId?'#16a34a':'#fff3',border:'none',color:'#fff',padding:'8px 11px',borderRadius:8,cursor:'pointer',fontWeight:600,fontSize:16,lineHeight:1}}>{emp.telegramChatId?'✓📱':'📱'}</button>
...
const doSave=useCallback(async nd=>{const prev=dataRef.current;stampUpdatedAt(prev&&prev.jobs,nd&&nd.jobs);savesInProgress.current++;savingRef.current=true;undoStack.current=[...(undoStack.current||[]).slice(-19),JSON.stringify(prev)];setData(nd);dataRef.current=nd;_liveData=nd;try{await teSyncChanges(prev,nd);await saveData(nd)}catch(e){console.warn('save error',e)}finally{setTimeout(()=>{savesInProgress.current=Math.max(0,savesInProgress.current-1);if(savesInProgress.current===0)savingRef.current=false},5000)}},[]);
