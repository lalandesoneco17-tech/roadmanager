...
const tgSend=async(token,chatId,text)=>{if(!token||!chatId)return{ok:false,error:'Token ou ID Telegram manquant'};try{const r=await fetch('https://api.telegram.org/bot'+token+'/sendMessage',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chat_id:chatId,text:text})});const j=await r.json();return j.ok?{ok:true}:{ok:false,error:j.description||'Erreur Telegram'}}catch(e){return{ok:false,error:String((e&&e.message)||e)}}};
