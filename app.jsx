...
const allPastJobsCam=(data.jobs||[]).filter(j2=>{if(j2.date<yearStart||j2.date>selDate||j2.type==='depot')return false;const e2=(data.employees||[]).find(e=>e.id===j2.employeeId);return e2&&e2.truckId===truckIdD&&j2.hasTransfer});
const revCamCum=allPastJobsCam.reduce((s2,j2)=>s2+(j2.transferPrice||0),0);
const coutsCamCum=(()=>{let cc=0;const allTrJobs=(data.jobs||[]).filter(j2=>{if(j2.date<yearStart||j2.date>selDate||j2.type==='depot')return false;const e2=(data.employees||[]).find(e=>e.id===j2.employeeId);return e2&&e2.truckId===truckIdD});allTrJobs.forEach(j2=>{const emp2=(data.employees||[]).find(e=>e.id===j2.employeeId);const hr2=emp2?Number(emp2.hourlySalary)||0:0;const cr2=emp2?Number(emp2.chargesRate)||45:45;const ft2=getMachineFuelType(data,j2.machineId);const fp2=getFuelPrice(data,ft2,j2.startFrom!=='home'?j2.startFrom:null);const trC2=truck2g?Number(truck2g.fuelPer100)||25:25;const m2=getMach(j2.machineId);const mt2=m2?m2.type:'';const trajL2=mt2==='Raboteuse'?((j2.distanceKm||0)/100)*trC2:(m2?(Number(m2.fuelConsumption)||0)*((j2.travelMin||0)/60):0);cc+=trajL2*fp2;cc+=((j2.travelMin||0)/60)*hr2*(1+cr2/100)});if(truck2g){const daysUsed=[...new Set(allTrJobs.map(j2=>j2.date))].length||1;cc+=((Number(truck2g.creditMonthly)||0)/wdpm)*daysUsed;cc+=((truck2g.insuranceMonthly||0)/wdpm)*daysUsed;cc+=(((truck2g.ctCost||0)/12)/wdpm)*daysUsed}return cc})();
