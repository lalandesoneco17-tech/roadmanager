...
<input type="time" value={uj.billingStart||'08:00'} onChange={e=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===uj.id);if(jj){jj.billingStart=e.target.value;save(nd)}}} style={{fontSize:15,padding:'4px 4px',borderRadius:6,border:'2px solid '+C.orange+'40',background:C.orange+'08',color:C.orange,fontWeight:700,width:75}}/>
...
<input type="time" value={j.billingStart||'08:00'} onChange={e=>{const nd=JSON.parse(JSON.stringify(_liveData||data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.billingStart=e.target.value;save(nd)}}} style={{fontSize:15,padding:'4px 4px',borderRadius:6,border:'2px solid '+C.orange+'40',background:C.orange+'08',color:C.orange,fontWeight:700,width:75}}/>
...
const doSave=useCallback(async nd=>{const prev=dataRef.current;stampUpdatedAt(prev&&prev.jobs,nd&&nd.jobs);_checkStockZero(prev,nd);savesInProgress.current++;savingRef.current=true;undoStack.current=[...(undoStack.current||[]).slice(-19),JSON.stringify(prev)];setData(nd);dataRef.current=nd;_liveData=nd;try{await teSyncChanges(prev,nd);if(nd&&nd._teDeletes)delete nd._teDeletes;await saveData(nd)}catch(e){console.warn('save error',e)}finally{setTimeout(()=>{savesInProgress.current=Math.max(0,savesInProgress.current-1);if(savesInProgress.current===0)savingRef.current=false},5000)}},[]);
