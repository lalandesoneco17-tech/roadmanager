...
<select onChange={e2=>{if(!e2.target.value)return;const nd=JSON.parse(JSON.stringify(data));if(!nd.jobs)nd.jobs=[];nd.jobs.push({...JSON.parse(JSON.stringify(uj)),id:uid(),employeeId:e2.target.value,sent:false,ack:false});save(nd);e2.target.value=''}} style={{fontSize:12,border:'2px solid '+C.cyan,borderRadius:6,padding:'3px 4px',background:'transparent',color:C.cyan,cursor:'pointer',fontWeight:600,width:70}} title="Ajouter un chauffeur"><option value="">+Chauff.</option>{(data.employees||[]).filter(e2=>e2.id!==(uj.employeeId||'')).map(e2=><option key={e2.id} value={e2.id}>{e2.name}</option>)}</select>
...
<div style={{marginLeft:'auto',display:'flex',gap:4,alignItems:'center'}}>
<select onChange={e2=>{if(!e2.target.value)return;const nd=JSON.parse(JSON.stringify(data));if(!nd.jobs)nd.jobs=[];nd.jobs.push({...JSON.parse(JSON.stringify(j)),id:uid(),employeeId:e2.target.value,sent:false,ack:false});save(nd);e2.target.value=''}} style={{fontSize:12,border:'2px solid '+C.cyan,borderRadius:6,padding:'3px 4px',background:'transparent',color:C.cyan,cursor:'pointer',fontWeight:600,width:70}} title="Ajouter un chauffeur sur ce chantier"><option value="">+Chauff.</option>{(data.employees||[]).filter(e2=>e2.id!==j.employeeId).map(e2=><option key={e2.id} value={e2.id}>{e2.name}</option>)}</select>
