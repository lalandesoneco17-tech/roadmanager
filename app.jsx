...
<h3 style={{margin:0,fontSize:16}}>🗺 Carte planning — {selDate} · {todayCount} chantier(s) <span style={{fontSize:10,color:C.dim,fontWeight:400,marginLeft:8}}>v2026.05.13-14</span></h3>
...
if(!j.startFrom||!j.endAt){setTimeout(()=>{const nd2=JSON.parse(JSON.stringify(data));const jj2=nd2.jobs.find(x=>x.id===j.id);if(!jj2)return;const empJobs=(nd2.jobs||[]).filter(jb=>jb.employeeId===jj2.employeeId&&jb.date===jj2.date&&jb.type!=='depot').sort((a,b)=>(a.billingStart||'99:99').localeCompare(b.billingStart||'99:99'));const myIdx=empJobs.findIndex(jb=>jb.id===jj2.id);const prevJob=myIdx>0?empJobs[myIdx-1]:null;const nextJob=myIdx>=0&&myIdx<empJobs.length-1?empJobs[myIdx+1]:null;let changed=false;if(!jj2.startFrom){const prevGps=prevJob?parseCoords(prevJob._geocodedGps||prevJob.gps):null;if(prevGps){const km=+(haversine(prevGps,gpsJ)*1.3).toFixed(0);jj2.startFrom=prevJob.id;jj2.kmAller=km;jj2.travelMinAller=Math.round((km/80)*60);changed=true}else if(shortDep){jj2.startFrom=shortDep.id;jj2.kmAller=shortDep.km;jj2.travelMinAller=Math.round((shortDep.km/80)*60);changed=true}}if(!jj2.endAt){const nextGps=nextJob?parseCoords(nextJob._geocodedGps||nextJob.gps):null;if(nextGps){const km=+(haversine(gpsJ,nextGps)*1.3).toFixed(0);jj2.endAt=nextJob.id;jj2.kmRetour=km;jj2.travelMinRetour=Math.round((km/80)*60);changed=true}else if(shortArr){jj2.endAt=shortArr.id;jj2.kmRetour=shortArr.km;jj2.travelMinRetour=Math.round((shortArr.km/80)*60);changed=true}}if(changed){jj2.distanceKm=(jj2.kmAller||0)+(jj2.kmRetour||0);jj2.travelMin=(jj2.travelMinAller||0)+(jj2.travelMinRetour||0);save(nd2)}},0)}
return null;
})()}
