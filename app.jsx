...
useEffect(()=>{if(!sb)return;const machNames=(data.machines||[]).map(m=>normJd(m.name)).filter(Boolean);if(!machNames.length)return;sb.from('machine_daily_reports').select('*').eq('report_date',selDate).in('jd_id',machNames).then(({data:rpts})=>{if(rpts)setJdReports(rpts)})},[selDate,data.machines]);
...
const doJdImport=async()=>{if(!sb){alert('Supabase requis');return;}const valid=jdImportRows.filter(r=>r.matched&&r.reportDate);if(!valid.length){alert('Aucune machine reconnue');return;}setJdImporting(true);setJdImportStatus('Import...');const records=valid.map(r=>({jd_id:r.jdId,report_date:r.reportDate,working_h:r.workingH,idle_h:r.idleH,transport_h:r.transportH,total_fuel_l:r.totalFuel,working_fuel_l:r.workingFuel,idle_fuel_l:r.idleFuel}));const dates=[...new Set(records.map(r=>r.report_date))];const jdIdsToDelete=[...new Set(records.map(r=>r.jd_id))];for(const dt of dates){await sb.from('machine_daily_reports').delete().eq('report_date',dt).in('jd_id',jdIdsToDelete);}const{error}=await sb.from('machine_daily_reports').insert(records);if(error){setJdImportStatus('Erreur: '+error.message);setJdImporting(false);return;}const importedDates=[...new Set(records.map(r=>r.report_date))];if(importedDates.includes(selDate)){const machNames2=(data.machines||[]).map(m=>normJd(m.name)).filter(Boolean);if(machNames2.length)sb.from('machine_daily_reports').select('*').eq('report_date',selDate).in('jd_id',machNames2).then(({data:rpts})=>{if(rpts)setJdReports(rpts)});}setJdImportStatus('✅ '+records.length+' rapport(s) importé(s) !');setJdImporting(false);setTimeout(()=>{setShowJdImport(false);setJdImportRows([]);setJdImportStatus('');},2500);};
