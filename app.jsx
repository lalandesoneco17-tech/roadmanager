...
const saveData=async(d)=>{localSave(d);if(sb){try{const{data:row}=await sb.from('app_data').select('data').eq('id','main').single();if(row&&row.data){const remote=row.data;const merged={...d};merged.timeEntries=mergeKeepLocal(d.timeEntries,remote.timeEntries);merged.panneReports=mergeKeepLocal(d.panneReports,remote.panneReports);merged.interventions=mergeKeepLocal(d.interventions,remote.interventions);merged.parts=mergeKeepLocal(d.parts,remote.parts);const{error}=await sb.from('app_data').upsert({id:'main',data:merged,updated_at:new Date().toISOString()});if(error)console.error('Supabase save error:',error);else{localSave(merged);console.log('Saved to Supabase (merged)')}}else{const{error}=await sb.from('app_data').upsert({id:'main',data:d,updated_at:new Date().toISOString()});if(error)console.error('Supabase save error:',error);else console.log('Saved to Supabase')}}catch(e){console.warn('Supabase save failed',e)}}};
...
const JD_FALLBACK={'18101314':'W130CFI','21200050':'W200FI'};
const handleJdFile=async(file)=>{if(!file)return;setJdImportStatus('Lecture...');setJdImportRows([]);let jdMachines=[];if(sb){const{data:jdData}=await sb.from('machines_jd').select('id,name');if(jdData)jdMachines=jdData;}const nameToId={};jdMachines.forEach(jd=>{nameToId[normJd(jd.name)]=String(jd.id)});const reader=new FileReader();reader.onload=(ev)=>{try{const wb=window.XLSX.read(ev.target.result,{type:'array'});const ws=wb.Sheets[wb.SheetNames[0]];const rows=window.XLSX.utils.sheet_to_json(ws,{header:1,defval:null});if(rows.length<2){setJdImportStatus('Fichier vide ou invalide');return;}const parsed=[];for(let i=1;i<rows.length;i++){const r=rows[i];const rawName=String(r[0]||'').trim();if(!rawName)continue;const normName=normJd(rawName);let jdId=nameToId[normName];if(!jdId){const fb=JD_FALLBACK[rawName]||JD_FALLBACK[normName];if(fb)jdId=nameToId[normJd(fb)];}const dateStr=String(r[6]||'').split(' ')[0];const dp=dateStr.split('/');const reportDate=dp.length===3?dp[2]+'-'+dp[1].padStart(2,'0')+'-'+dp[0].padStart(2,'0'):'';const workingH=r[10]!=null?+Number(r[10]).toFixed(2):null;const idleH=r[11]!=null?+Number(r[11]).toFixed(2):null;const motorH=r[8]!=null?+Number(r[8]).toFixed(2):null;const transportH=(motorH!=null&&workingH!=null&&idleH!=null)?+Math.max(0,motorH-workingH-idleH).toFixed(2):null;const totalFuel=r[12]!=null?+Number(r[12]).toFixed(0):null;const workingFuel=r[15]!=null?+Number(r[15]).toFixed(0):null;const idleFuel=r[16]!=null?+Number(r[16]).toFixed(0):null;parsed.push({rawName,jdId:jdId||null,matched:!!jdId,reportDate,workingH,idleH,transportH,totalFuel,workingFuel,idleFuel})}setJdImportRows(parsed);setJdImportStatus('');}catch(err){setJdImportStatus('Erreur: '+err.message);}};reader.readAsArrayBuffer(file);};
const doJdImport=async()=>{if(!sb){alert('Supabase requis');return;}const valid=jdImportRows.filter(r=>r.matched&&r.reportDate);if(!valid.length){alert('Aucune machine reconnue');return;}setJdImporting(true);setJdImportStatus('Import...');const records=valid.map(r=>({jd_id:r.jdId,report_date:r.reportDate,working_h:r.workingH,idle_h:r.idleH,transport_h:r.transportH,total_fuel_l:r.totalFuel,working_fuel_l:r.workingFuel,idle_fuel_l:r.idleFuel}));const{error}=await sb.from('machine_daily_reports').upsert(records,{onConflict:'jd_id,report_date'});if(error){setJdImportStatus('Erreur: '+error.message);setJdImporting(false);return;}const importedDates=[...new Set(records.map(r=>r.report_date))];if(importedDates.includes(selDate)){const jdIds2=(data.machines||[]).filter(m=>m.jdId).map(m=>m.jdId);if(jdIds2.length)sb.from('machine_daily_reports').select('*').eq('report_date',selDate).in('jd_id',jdIds2).then(({data:rpts})=>{if(rpts)setJdReports(rpts)});}setJdImportStatus('✅ '+records.length+' rapport(s) importé(s) !');setJdImporting(false);setTimeout(()=>{setShowJdImport(false);setJdImportRows([]);setJdImportStatus('');},2500);};
...
<select onChange={e2=>{if(!e2.target.value)return;const selEmp2=(data.employees||[]).find(x=>x.id===e2.target.value);const nd=JSON.parse(JSON.stringify(data));if(!nd.jobs)nd.jobs=[];const nj={...JSON.parse(JSON.stringify(uj)),id:uid(),employeeId:e2.target.value,machineId:selEmp2?selEmp2.machineId||uj.machineId:uj.machineId,sent:false,ack:false};nd.jobs.push(nj);save(nd);e2.target.value=''}} style={{fontSize:12,border:'2px solid '+C.cyan,borderRadius:6,padding:'3px 4px',background:'transparent',color:C.cyan,cursor:'pointer',fontWeight:600,width:40}} title="Ajouter un chauffeur"><option value="">+C</option>{(data.employees||[]).filter(e2=>e2.id!==(uj.employeeId||'')).map(e2=><option key={e2.id} value={e2.id}>{e2.name}</option>)}</select>
...
return(<React.Fragment>
<select value={j.startFrom||''} onChange={e=>{const nd=JSON.parse(JSON.stringify(data));const jj=nd.jobs.find(x=>x.id===j.id);if(jj){jj.startFrom=e.target.value;const sel2=depOptions2.find(o=>o.id===e.target.value);jj.kmAller=sel2&&sel2.km?sel2.km:0;jj.travelMinAller=sel2&&sel2.km?Math.round((sel2.km/80)*60):0;jj.distanceKm=(jj.kmAller||0)+(jj.kmRetour||0);jj.travelMin=(jj.travelMinAller||0)+(jj.travelMinRetour||0);save(nd)}}} style={{fontSize:12,padding:'2px 3px',borderRadius:4,border:'1px solid #0891b240',background:j.startFrom&&shortDep&&j.startFrom===shortDep.id?'#0891b218':'#0891b208',color:'#0891b2',fontWeight:600,minWidth:60,maxWidth:110}}>
<option value="">↗Dep</option>{depOptions2.map(o=><option key={o.id} value={o.id}>{o.name}{o.km!==null?' '+o.km+'km':''}{shortDep&&o.id===shortDep.id?' ★':''}</option>)}
</select>
...
<div style={{marginLeft:'auto',display:'flex',gap:4,alignItems:'center'}}>
<select onChange={e2=>{if(!e2.target.value)return;const selEmp2=(data.employees||[]).find(x=>x.id===e2.target.value);const nd=JSON.parse(JSON.stringify(data));if(!nd.jobs)nd.jobs=[];const nj={...JSON.parse(JSON.stringify(j)),id:uid(),employeeId:e2.target.value,machineId:selEmp2?selEmp2.machineId||j.machineId:j.machineId,sent:false,ack:false};nd.jobs.push(nj);save(nd);e2.target.value=''}} style={{fontSize:12,border:'2px solid '+C.cyan,borderRadius:6,padding:'3px 4px',background:'transparent',color:C.cyan,cursor:'pointer',fontWeight:600,width:40}} title="Ajouter un chauffeur sur ce chantier"><option value="">+C</option>{(data.employees||[]).filter(e2=>e2.id!==j.employeeId).map(e2=><option key={e2.id} value={e2.id}>{e2.name}</option>)}</select>
