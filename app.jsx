...
<div style={{position:'relative',flex:1,minWidth:140,display:'flex'}}><input value={j.location||''} onChange={e=>{const v=e.target.value;const tgt=e.target;modif(j.id,jj=>{jj.location=v;if(jj._geocodedGps)jj._geocodedGps='';const hh=heureDe(v);if(hh)jj.billingStart=hh});triggerAutoGeocode(j.id,v,tgt)}} onFocus={e=>{if(j.location&&j.location.trim().length>=3&&!j._geocodedGps)triggerAutoGeocode(j.id,j.location,e.target)}} title={gps?'GPS : '+gps:'Lieu (tape l heure dedans : ... a 21h)'} style={{...boxS(0,rouge),width:'100%',paddingLeft:gps?24:7,paddingRight:fin?160:62}}/>{gps&&<span style={{position:'absolute',left:6,top:6,fontSize:13,lineHeight:1,pointerEvents:'none'}}>📍</span>}{/* l'heure : lue dans le lieu, ou saisie ici directement ; a sa droite, la fin de chantier pointee et le temps passe (23/09/2026) */}<div style={{position:'absolute',right:4,top:4,height:H-8,display:'flex',alignItems:'center',gap:4}}><input type="time" className="rm-heure" value={h||''} onChange={e=>{const v=e.target.value||'';modif(j.id,jj=>{jj.billingStart=v})}} title="Heure de début du chantier" style={{height:H-8,fontSize:11,fontWeight:700,color:'#0f766e',background:h?'#ccfbf1':'#f1f5f9',border:'none',borderRadius:4,padding:'0 4px',lineHeight:1.2,width:52,boxSizing:'border-box',fontFamily:'inherit',cursor:'pointer'}}/>{fin&&<span title="Fin de chantier pointée par le chauffeur · temps passé" style={{height:H-8,display:'inline-flex',alignItems:'center',fontSize:11,fontWeight:700,color:'#15803d',background:'#dcfce7',borderRadius:4,padding:'0 5px',whiteSpace:'nowrap'}}>{fin}</span>}</div></div>
...
const ligneVide=(m,empId,k)=>(<div key={'v'+k} style={{display:'flex',alignItems:'center',gap:5,padding:'3px 6px',borderBottom:'1px solid '+C.border+'66',height:H+8,boxSizing:'border-box'}}>
<Coche on={false} color={C.red} label="nuit" onClick={()=>nouveauJob(m,empId,'isNight',true)}/>
<input list="planning-clients-list" onBlur={e=>{const v=e.target.value.trim();if(!v)return;const nd=JSON.parse(JSON.stringify(_liveData||data));if(!nd.clients)nd.clients=[];let c=nd.clients.find(c2=>c2.name.toLowerCase()===v.toLowerCase());if(!c){c={id:uid(),name:v,forfaitType:'standard',agencies:[],siteManagers:[]};nd.clients.push(c)}if(!nd.jobs)nd.jobs=[];nd.jobs.push({...gabarit(m,empId),clientId:c.id});save(nd);e.target.value=''}} style={boxS(118)}/>
<select disabled style={boxS(108,{padding:'2px 4px',background:'#f8fafc'})}><option value=""></option></select>
<input onBlur={e=>{const v=e.target.value.trim();if(v){nouveauJob(m,empId,'location',v);e.target.value=''}}} onKeyDown={e=>{if(e.key==='Enter'&&e.target.value.trim()){nouveauJob(m,empId,'location',e.target.value.trim());e.target.value=''}}} style={boxS(0)}/>
<select value="" onChange={e=>{if(e.target.value)nouveauJob(m,empId,'forfaitType',e.target.value)}} style={boxS(50,{padding:'2px 2px',textAlign:'center'})}><option value=""></option>{(m&&m.type==='Citerne'?['Demi-journee','Journee']:['2h','4h','6h','8h','10h']).map(f=><option key={f} value={f}>{f}</option>)}</select>
<Coche on={false} color="#ca8a04" label="bon"/>
...
const colPaye=(<div style={{width:26,minWidth:26,display:'flex',flexDirection:'column',borderRight:'1px solid '+col+'30',background:col+'08'}}>{rows.map((r,i)=>(<div key={'p'+i} style={{height:H+8,display:'flex',alignItems:'center',justifyContent:'center'}}>{r.job?<span title={r.job.paid?'Client a payé':'Marquer client payé'} onClick={()=>modif(r.job.id,jj=>{jj.paid=!jj.paid})} style={{display:'inline-flex',width:18,height:18,borderRadius:4,background:r.job.paid?C.purple:'#fff',border:'2px solid '+(r.job.paid?C.purple:C.muted),boxSizing:'border-box',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:12,fontWeight:800,cursor:'pointer'}}>{r.job.paid?'€':''}</span>:null}</div>))}</div>);
...
<div ref={fitRef} onClick={()=>{if(showFont)setShowFont(false)}} style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,alignItems:'start',fontFamily:TF.family}}>
...
return(
<div>
<h2 style={{marginBottom:4}}>📋 Recap heures — tous les chauffeurs <span style={{fontSize:10,color:C.dim,fontWeight:400,marginLeft:8}}>v2026.09.24-5</span></h2>
