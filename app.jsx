...
<div style={{position:'relative',flex:1,minWidth:140,display:'flex'}}><input value={j.location||''} onChange={e=>{const v=e.target.value;const tgt=e.target;modif(j.id,jj=>{jj.location=v;if(jj._geocodedGps)jj._geocodedGps='';const hh=heureDe(v);if(hh)jj.billingStart=hh});triggerAutoGeocode(j.id,v,tgt)}} onFocus={e=>{if(j.location&&j.location.trim().length>=3&&!j._geocodedGps)triggerAutoGeocode(j.id,j.location,e.target)}} title={gps?'GPS : '+gps:'Lieu (tape l heure dedans : ... a 21h)'} style={{...boxS(0,rouge),width:'100%',paddingLeft:gps?24:7,paddingRight:70}}/>{gps&&<span style={{position:'absolute',left:6,top:6,fontSize:13,lineHeight:1,pointerEvents:'none'}}>📍</span>}{/* l'heure : lue dans le lieu, ou saisie ici directement (23/09/2026) */}<input type="time" value={h||''} onChange={e=>{const v=e.target.value||'';modif(j.id,jj=>{jj.billingStart=v})}} title="Heure de début du chantier" style={{position:'absolute',right:4,top:4,height:H-8,fontSize:11,fontWeight:700,color:'#0f766e',background:h?'#ccfbf1':'#f1f5f9',border:'none',borderRadius:4,padding:'0 4px',lineHeight:1.2,width:62,boxSizing:'border-box',fontFamily:'inherit',cursor:'pointer'}}/></div>
...
return(
<div>
<h2 style={{marginBottom:4}}>📋 Recap heures — tous les chauffeurs <span style={{fontSize:10,color:C.dim,fontWeight:400,marginLeft:8}}>v2026.09.23-10</span></h2>
