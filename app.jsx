...
const machs=(data.machines||[]).filter(m=>types.includes(m.type)).sort((a,b)=>(types.indexOf(a.type)-types.indexOf(b.type))||(a.type==='Raboteuse'?(wOf(b)-wOf(a)):0)||String(a.name||'').localeCompare(String(b.name||'')));
...
const H=30;
const boxS=(w,extra)=>({fontSize:14,padding:'2px 7px',borderRadius:5,border:'1px solid '+C.border,background:'#fff',height:H,boxSizing:'border-box',display:'flex',alignItems:'center',gap:5,width:w||undefined,flex:w?'none':1,minWidth:w?undefined:140,color:C.text,whiteSpace:'nowrap',overflow:'hidden',outline:'none',...(extra||{})});
...
return(<div key={j.id} style={{display:'flex',alignItems:'center',gap:5,padding:'3px 6px',borderBottom:'1px solid '+C.border+'66',background:bg,height:H+8,boxSizing:'border-box'}}>
...
const bloc=(empId,js)=>{const emp=getEmp(empId);const prev=js.length>0&&js.every(j=>j.sent);return(<div style={{flex:1,display:'flex',alignItems:'center',gap:6,padding:'0 6px',background:prev?'#00FF00':'transparent',minWidth:0}}>{emp?<React.Fragment>{btnPrev(js)}<div title={emp.name} style={{fontSize:16,fontWeight:800,color:C.text,lineHeight:1.1,whiteSpace:'nowrap',overflow:'hidden'}}>{emp.name}</div></React.Fragment>:<div onClick={e=>{e.stopPropagation();setAddEmpOpen(addEmpOpen===pickKey?null:pickKey)}} title="Choisir le chauffeur" style={{width:96,height:24,border:'1px solid '+C.border,borderRadius:5,background:'#fff',cursor:'pointer'}}></div>}</div>)};
...
return(<div key={mid||'sans'} style={{background:'#fff',borderRadius:8,border:'2px solid '+col,borderLeft:'6px solid '+col,boxShadow:'0 2px 6px rgba(0,0,0,.06)',display:'flex',marginBottom:5,position:'relative'}}>{colPaye}{gauche}<div style={{flex:1,minWidth:0}}>{rows.map((r,i)=>r.job?ligneJob(r.job,m):(r.dep?ligneDepot(r.dep):ligneVide(m,drivers[0]||'',i)))}</div></div>)};
const sansMachine=types.includes('Raboteuse')?dayMissions.filter(j=>!getMach(j.machineId)):[];
return(<div>
<div style={{display:'flex',alignItems:'center',gap:8,margin:'2px 0 6px'}}><span style={{color:'#fff',fontWeight:800,fontSize:13,letterSpacing:.4,textTransform:'uppercase'}}>{label}</span><span style={{flex:1,height:1,background:'#ffffff33'}}></span><span style={{color:'#fff',fontWeight:700,fontSize:13}}>{fmtMoney(caCol)}</span></div>
...
<div ref={fitRef} style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,alignItems:'start'}}>
...
return(
<div>
<h2 style={{marginBottom:4}}>📋 Recap heures — tous les chauffeurs <span style={{fontSize:10,color:C.dim,fontWeight:400,marginLeft:8}}>v2026.09.23-11</span></h2>
