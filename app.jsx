...
<h3 style={{margin:0,fontSize:16}}>🗺 Carte planning — {selDate} · {todayCount} chantier(s) <span style={{fontSize:10,color:C.dim,fontWeight:400,marginLeft:8}}>v2026.05.13-8</span></h3>
...
<input list="planning-clients-list" placeholder="Client" value={ujCl?ujCl.name:''} onChange={e=>{const v=e.target.value;const matched=(data.clients||[]).find(c2=>c2.name===v);const nd=JSON.parse(JSON.stringify(data));const jj=nd.jobs.find(x=>x.id===uj.id);if(!jj)return;if(matched){jj.clientId=matched.id;if(ujM&&jj.forfaitType){const p=getForfaitPrice(nd,matched.id,ujM,jj.forfaitType,jj.citOption,jj.isNight);if(p)jj.priceForfait=p}save(nd)}else if(!v){jj.clientId='';save(nd)}}} onKeyDown={e=>{if(e.key==='Enter'){const v=e.target.value.trim();if(!v)return;const exists=(data.clients||[]).find(c2=>c2.name.toLowerCase()===v.toLowerCase());if(exists)return;if(confirm('Creer le client "'+v+'" ?')){const nd=JSON.parse(JSON.stringify(data));if(!nd.clients)nd.clients=[];const nc={id:uid(),name:v,forfaitType:'standard',agencies:[],siteManagers:[]};nd.clients.push(nc);const jj=nd.jobs.find(x=>x.id===uj.id);if(jj)jj.clientId=nc.id;save(nd);e.target.blur()}}}} style={{fontSize:15,padding:'4px 6px',borderRadius:6,border:'1px solid '+C.border,background:'#fff',minWidth:100,maxWidth:150}}/>
...
<div key={eId+'_'+(grp.m?grp.m.id:'none')} onDragOver={e2=>{e2.preventDefault();if(dragJobId){if(eId!==dragJobOverEmp)setDragJobOverEmp(eId)}else if(dragId&&cardId!==dragId){setDragOverId(cardId)}}} onDrop={e2=>{if(dragJobId)onJobDrop(e2,eId)}} style={{background:allAck?'#dcfce7':C.card,borderRadius:10,marginBottom:12,marginTop:(showSep&&grpIdx===0)?28:0,border:'3px solid '+(dragJobOverEmp===eId?C.cyan+'CC':dragOverId===cardId?C.accent+'80':allAck?'#16a34a':machColor),borderLeft:'6px solid '+(allAck?C.green:machColor),overflow:'hidden',boxShadow:'0 2px 6px rgba(0,0,0,.06)',display:'flex',opacity:dragId===cardId?0.5:1}}>
...
<input list="planning-clients-list" placeholder="Client" value={cl?cl.name:''} onChange={e=>{const v=e.target.value;const matched=(data.clients||[]).find(c2=>c2.name===v);const nd=JSON.parse(JSON.stringify(data));const jj=nd.jobs.find(x=>x.id===j.id);if(!jj)return;if(matched){jj.clientId=matched.id;const m3=getMach(jj.machineId);if(m3&&jj.forfaitType){const p=getForfaitPrice(nd,matched.id,m3,jj.forfaitType,jj.citOption,jj.isNight);if(p)jj.priceForfait=p}save(nd)}else if(!v){jj.clientId='';save(nd)}}} onKeyDown={e=>{if(e.key==='Enter'){const v=e.target.value.trim();if(!v)return;const exists=(data.clients||[]).find(c2=>c2.name.toLowerCase()===v.toLowerCase());if(exists)return;if(confirm('Creer le client "'+v+'" ?')){const nd=JSON.parse(JSON.stringify(data));if(!nd.clients)nd.clients=[];const nc={id:uid(),name:v,forfaitType:'standard',agencies:[],siteManagers:[]};nd.clients.push(nc);const jj=nd.jobs.find(x=>x.id===j.id);if(jj)jj.clientId=nc.id;save(nd);e.target.blur()}}}} style={{fontSize:15,padding:'4px 6px',borderRadius:6,border:'2px solid '+(j.paid?C.purple:C.border),background:j.paid?C.purple+'20':'#fff',color:j.paid?C.purple:'inherit',fontWeight:j.paid?700:400,minWidth:100,maxWidth:150}}/>
...
{/* Ligne Wirtgen résumé global (déplacée ici depuis au-dessus de la timeline) */}
...
return(<div style={{padding:'4px 8px',display:'flex',alignItems:'center',gap:5,flexWrap:'wrap',background:'#fefce8',fontSize:11,borderRadius:6,border:'1px solid #fde68a',marginBottom:6}}>
