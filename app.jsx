...
<div style={{width:75,minWidth:75,maxWidth:75,padding:'8px 4px',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',borderRight:'2px solid '+(allAck?'#16a34a20':machColor+'20'),background:machColor+'08',gap:3}}>
<div style={{display:'flex',alignItems:'center',gap:3,justifyContent:'center'}}>
<div style={{fontSize:13,fontWeight:800,color:C.text,textAlign:'center',lineHeight:'1.1'}}>{emp.name}</div>
<button onClick={e=>{e.stopPropagation();const nd=JSON.parse(JSON.stringify(data));if(!nd.jobs)nd.jobs=[];nd.jobs.push({id:uid(),date:selDate,employeeId:eId,machineId:grp.m?grp.m.id:emp.machineId||'',clientId:'',agencyName:'',siteManager:'',siteManagerPhone:'',location:'',gps:'',forfaitType:'',priceForfait:0,isNight:false,hasTransfer:false,transferPrice:0,billingStart:'08:00',startFrom:'',endAt:'',machineFuelL:0,machineFuelDepot:'',kmAller:0,kmRetour:0,travelMinAller:0,travelMinRetour:0,distanceKm:0,travelMin:0,sent:false});save(nd)}} title="Ajouter un chantier" style={{background:C.accent,color:'#fff',border:'none',borderRadius:4,width:18,height:18,cursor:'pointer',fontSize:13,fontWeight:700,lineHeight:'16px',padding:0,flexShrink:0}}>+</button>
</div>
<div style={{fontSize:11,fontWeight:700,color:machColor,textAlign:'center',lineHeight:'1.1'}}>{grp.m?grp.m.name:'?'}</div>
{(()=>{const dowN=new Date(selDate).getDay();const dfmN=dowN===0?6:dowN-1;const monN=new Date(selDate);monN.setDate(monN.getDate()-dfmN);const monISO=fmtDateISO(monN);const wkTEs=(data.timeEntries||[]).filter(te2=>te2.empId===eId&&te2.date>=monISO&&te2.date<=selDate);const wkDates=[...new Set(wkTEs.filter(te2=>te2.startTime&&te2.endTime).map(te2=>te2.date))];const weekMin=wkDates.reduce((s,d)=>{const best=wkTEs.find(te2=>te2.date===d&&te2.startTime&&te2.endTime);if(!best)return s;const[sh2,sm2]=best.startTime.split(':').map(Number);const[eh2,em2]=best.endTime.split(':').map(Number);return s+Math.max(0,(eh2*60+em2)-(sh2*60+sm2)-(best.pauseMin||0))},0);const dayMin=(mainTE&&mainTE.startTime&&mainTE.endTime)?calcWorkedMin(mainTE):workMin;if(dayMin<=0&&weekMin<=0)return null;return(<div style={{fontSize:9,color:C.dim,textAlign:'center',marginTop:2,lineHeight:1.2,background:'#f1f5f9',borderRadius:5,padding:'2px 3px',width:'100%'}}><div>J <b style={{color:C.accent,fontSize:10}}>{fmtDuration(dayMin)}</b></div><div>S <b style={{color:C.accent,fontSize:10}}>{fmtDuration(weekMin)}</b></div></div>)})()}
{isMonthly&&<div style={{fontSize:9,color:C.dim,textAlign:'center'}}>{fmtMoney(dailySalary)}/j</div>}
</div>
